...
# =====================================================================
# Section 25: SKIP-RISK-EVENTS-WIPE-1 回帰テスト
# --skip-risk実行時、既存のrisk_eventsが無条件で空配列[]に上書きされる
# バグの修正確認。_save_result()冒頭でlatest.json中間保存前に既存値を
# 退避し、skip_risk=True時はその退避値を引き継ぐ設計とした。
# =====================================================================

class TestSkipRiskEventsPreserved:
    """--skip-risk実行時のrisk_events保全（SKIP-RISK-EVENTS-WIPE-1）

    このテストファイル冒頭でgrowth_sanityモジュール全体がMagicMockに
    差し替えられているため（本物のロジックはテスト6でのみ_gs経由で検証）、
    pipeline.check_growth_sanity（pipeline.py::_save_result内部で呼ばれる）
    を素の空dictを返すようmonkeypatchしてから_save_result()を呼ぶ
    （空dictなら`if _growth_sanity:`がFalseとなり、後続の
    `_fcf_margin_bear_mult < 1.0`比較がMagicMockではなくデフォルト値1.0の
    floatで行われ、本テストの対象外であるgrowth_sanity関連コードパスで
    無関係なTypeErrorが起きるのを避けられる）。"""

    def test_skip_risk_preserves_existing_risk_events(self, tmp_path, monkeypatch):
        """既存latest.jsonにrisk_eventsがある場合、--skip-risk実行後も
        そのまま引き継がれること（中間保存で自分自身が上書きした空の
        状態を読んでしまう回帰の再発防止）"""
        import json, os
        monkeypatch.setattr(pipeline, "check_growth_sanity", lambda *a, **k: {})
        pipe = _make_pipe(tmp_path)
        pipe.skip_risk = True
        ticker_dir = os.path.join(pipe.output_dir, "TESTCO")
        os.makedirs(ticker_dir, exist_ok=True)
        existing_events = [{"title": "既存リスクイベント", "severity": "high"}]
        with open(os.path.join(ticker_dir, "latest.json"), "w", encoding="utf-8") as f:
            json.dump({"risk_events": existing_events}, f)

        pipe._save_result("TESTCO", _minimal_valuation())

        with open(os.path.join(ticker_dir, "latest.json"), encoding="utf-8") as f:
            saved = json.load(f)
        assert saved.get("risk_events") == existing_events, (
            "SKIP-RISK-EVENTS-WIPE-1 回帰: --skip-risk実行で既存risk_eventsが"
            "保持されていない"
        )

    def test_skip_risk_new_ticker_defaults_to_empty_list(self, tmp_path, monkeypatch):
        """既存latest.jsonが存在しない（新規銘柄相当）場合は従来通り
        空配列のままであること"""
        import json, os
        monkeypatch.setattr(pipeline, "check_growth_sanity", lambda *a, **k: {})
        pipe = _make_pipe(tmp_path)
        ...
        pipe._save_result("NEWCO", _minimal_valuation())

        latest_path = os.path.join(pipe.output_dir, "NEWCO", "latest.json")
        with open(latest_path, encoding="utf-8") as f:
            saved = json.load(f)
        assert saved.get("risk_events") == []

    def test_skip_risk_missing_risk_events_key_defaults_to_empty_list(self, tmp_path, monkeypatch):
        """既存latest.jsonは存在するがrisk_eventsキー自体がない場合も
        空配列のままであること"""
        import json, os
        monkeypatch.setattr(pipeline, "check_growth_sanity", lambda *a, **k: {})
        pipe = _make_pipe(tmp_path)
        pipe.skip_risk = True
        ticker_dir = os.path.join(pipe.output_dir, "TESTCO2")
        os.makedirs(ticker_dir, exist_ok=True)
        with open(os.path.join(ticker_dir, "latest.json"), "w", encoding="utf-8") as f:
            json.dump({"tanuki_score": "BUY"}, f)

        pipe._save_result("TESTCO2", _minimal_valuation())
