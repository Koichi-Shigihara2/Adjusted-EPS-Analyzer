...
# ─────────────────────────────────────────────
# 8. HypeCoreフェーズ重み付き推奨成長率
#    Phase1→TTM重視, Phase3→業界平均寄り, Phase4→最も保守的
# ─────────────────────────────────────────────

class TestHypecoreWeightedGrowth:
    """HypeCoreフェーズ重み付き recommended_g のテスト。
    すべて同じTTM・業界ベンチマーク・historicalを使い、フェーズ差だけを確認する。
    setup: TTM=40%, industry=10%, annual_revenues で cagr_3yr≈11.87%
    """

    _COMMON_REVENUES = [1000.0, 1200.0, 1300.0, 1400.0]  # cagr_3yr ≈ 11.87%

    @patch.object(_gs, "get_industry_benchmark", return_value={
        "industry": "Software (System & Application)",
        "g_ebit": 0.10, "roc": 0.20, "rr": 0.60,
    })
    def test_phase1_higher_than_phase3(self, _mock):
        """Phase1（黎明期）→ TTM重視で高め、Phase3（陶酔期）より大きい"""
        r1 = _gs.check_growth_sanity(
            "TEST", phase1_growth=0.40, sector="Software_System",
            annual_revenues=self._COMMON_REVENUES,
            hypecore_stage=1, ttm_rev_yoy=0.40,
        )
        r3 = _gs.check_growth_sanity(
            "TEST", phase1_growth=0.40, sector="Software_System",
            annual_revenues=self._COMMON_REVENUES,
            hypecore_stage=3, ttm_rev_yoy=0.40,
        )
        assert r1["recommended_g_hypecore"] is not None
        assert r3["recommended_g_hypecore"] is not None
        assert r1["hypecore_stage_used"] == 1
        assert r3["hypecore_stage_used"] == 3
        assert r1["recommended_g_hypecore"] > r3["recommended_g_hypecore"]

    @patch.object(_gs, "get_industry_benchmark", return_value={
        "industry": "Software (System & Application)",
        "g_ebit": 0.10, "roc": 0.20, "rr": 0.60,
    })
    def test_phase4_most_conservative(self, _mock):
        """Phase4（期待剥落期）→ 最も保守的（recommended_g_hypecore が最低）"""
        r3 = _gs.check_growth_sanity(
            "TEST", phase1_growth=0.40, sector="Software_System",
            annual_revenues=self._COMMON_REVENUES,
            hypecore_stage=3, ttm_rev_yoy=0.40,
        )
        r4 = _gs.check_growth_sanity(
            "TEST", phase1_growth=0.40, sector="Software_System",
            annual_revenues=self._COMMON_REVENUES,
            hypecore_stage=4, ttm_rev_yoy=0.40,
        )
        assert r4["recommended_g_hypecore"] is not None
        assert r4["recommended_g_hypecore"] < r3["recommended_g_hypecore"]

    @patch.object(_gs, "get_industry_benchmark", return_value={
        "industry": "Software (System & Application)",
        "g_ebit": 0.10, "roc": 0.20, "rr": 0.60,
    })
    def test_phase3_benchmark_heavy_with_high_ttm(self, _mock):
        """Phase3（陶酔期）は benchmark 重視 → 高TTMでも推奨値が業界平均寄りに抑えられる"""
        # TTM=50%, benchmark=10% → Phase3 の recommended_g_hypecore < 50%
        r3 = _gs.check_growth_sanity(
            "TEST", phase1_growth=0.50, sector="Software_System",
            annual_revenues=self._COMMON_REVENUES,
            hypecore_stage=3, ttm_rev_yoy=0.50,
        )
        assert r3["recommended_g_hypecore"] is not None
        # Phase3 は benchmark(10%)寄り → 50% × 0.2 = 10% 以下には下がらないが
        # TTM 50% より大幅に低くなるはず（benchmark=10%が重みの50%を占める）
        assert r3["recommended_g_hypecore"] < 0.50

    @patch.object(_gs, "get_industry_benchmark", return_value={
        "industry": "Software (System & Application)",
        "g_ebit": 0.10, "roc": 0.20, "rr": 0.60,
    })
    def test_no_hypecore_stage_uses_median_only(self, _mock):
        """hypecore_stage=None → recommended_g_hypecore=None, recommended_g=中央値"""
        result = _gs.check_growth_sanity(
            "TEST", phase1_growth=0.40, sector="Software_System",
            annual_revenues=self._COMMON_REVENUES,
            hypecore_stage=None, ttm_rev_yoy=None,
        )
        assert result["recommended_g_hypecore"] is None
        assert result["hypecore_stage_used"] is None
        assert result["recommended_g_median"] is not None
        # hypecore なし → recommended_g == median
        assert result["recommended_g"] == result["recommended_g_median"]
