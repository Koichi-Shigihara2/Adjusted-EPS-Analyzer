...
- 詳細なデバッグ出力とエラーハンドリング
- ★ 税引前利益が直接取得できない場合、net_income + tax_expense から計算する処理を追加
"""
import os
import csv
import json
...
def extract_quarterly_facts(ticker: str, years: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # ★★★ 税引前利益の計算（net_income + tax_expense）★★★
        # 各四半期データに net_income と tax_expense があれば pretax_income を計算して追加
        for key, data in quarters_map.items():
            # 既に pretax_income が直接存在する場合はスキップ
            if 'pretax_income' in data:
                continue
            net_income = data.get('net_income')
            tax_expense = data.get('us-gaap:IncomeTaxExpenseBenefit')  # tax_expense タグの値
            if net_income and tax_expense:
                ...
                print(f"  Computed pretax_income for {data['filing_date']}: {pretax_val:,.0f} USD (net={net_val:,.0f}, tax={tax_val:,.0f})")
