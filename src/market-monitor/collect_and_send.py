...
def extract_judgment(report_text):
    # 「判定：曇り」「判定：晴れ」「判定：嵐」を正確に抽出
    # [晴曇嵐]+ では「り」「れ」が除外されて「曇」「晴」になるバグを修正
    for keyword in ["嵐", "曇り", "晴れ"]:
        if f"判定：{keyword}" in report_text[:300]:  # 冒頭300文字を優先チェック
            return keyword
    match = re.search(r'判定：(嵐|曇り|晴れ)', report_text)
