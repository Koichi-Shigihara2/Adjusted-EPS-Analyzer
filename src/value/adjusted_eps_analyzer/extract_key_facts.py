...
- ★ 四半期判別閾値を緩和（60〜140日）、年次閾値を280日に変更（データ欠落防止）
- ★ Q4計算時にQ1-3のいずれかが欠損している場合はスキップ（安全化）
"""
import os
import csv
import json
...
# ★ 閾値緩和（データ欠落防止）
QUARTER_DAYS_MIN = 60     # 2ヶ月程度の短い四半期にも対応
QUARTER_DAYS_MAX = 140    # 4ヶ月半まで許容（会計期間変更時など）
ANNUAL_DAYS_MIN = 280     # 11ヶ月以上（IPO直後の短い年次にも対応）
...
def extract_quarterly_facts(ticker: str, years: int = 10) -> List[Dict[str, Any]]:
    try:
        ...
        for fiscal_year in fiscal_years_with_q1q3:
            ...
            # ★ Q1-3 のいずれかが欠損している場合は Q4 を計算しない（安全化）
            if not (q1_key in quarters_map and q2_key in quarters_map and q3_key in quarters_map):
                print(f"  [Q4計算スキップ] FY{fiscal_year}: Q1-3のいずれかが欠損しています")
                continue
            ...
            if not target_k_item:
                print(f"  [Q4計算スキップ] FY{fiscal_year}: 対応する10-Kが見つかりません")
                continue
