...
# 定数設定
# ============================================
# ファイル位置: src/value/adjusted_eps_analyzer/extract_key_facts.py
# プロジェクトルート: 3階層上
...
# デバッグ出力
print(f"[extract_key_facts] PROJECT_ROOT = {PROJECT_ROOT}")
...
def normalize_value(value_dict) -> float:
    ...
    # スカラー値の場合
    if isinstance(value_dict, (int, float)):
        return float(value_dict)
    if isinstance(value_dict, str):
        try:
            return float(value_dict.replace(',', ''))
