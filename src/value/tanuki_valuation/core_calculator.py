...
class KoichiValuationCalculator:
    def __init__(self):
        self.wacc = 0.085          # 割引率
        self.k = 0.10              # モメンタム係数
        self.high_growth_years = 3 # KPI監視期間
        self.retention_rate = 0.60 # 内部留保率（簡易値）
    ...
    def calculate_pt(self, financials: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # === α_個別成長期待（本格Koichi式v5.1） ===
        g_individual = max(0.0, roe_avg * self.retention_rate)           # 超過成長率
        alpha = max(0.0, (g_individual / self.wacc) * 0.7)               # α_個別
        beta = 0.0
        m_total = 0.0

        print(f"   [{ticker}] ROE_10yr_avg = {roe_avg:.1%} → α = {alpha:.3f}")
        ...
        return {
            ...
            "components": {
                **financials,
                ...
                "retention_rate_used": float(self.retention_rate)
            }
        }