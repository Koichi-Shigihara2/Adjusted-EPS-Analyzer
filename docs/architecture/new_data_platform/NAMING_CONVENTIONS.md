...
## 適用範囲の明確化（2026-08-03追記）

本ドキュメントの規則1〜7は、**(a) 同一の経済概念に対し複数サブシステムが
異なる計算ロジック・異なる定義（データソース・期間）を与えている場合**
にのみ適用される（問題パターンA/Bそのもの）。BACKLOG.md「重複計算
パターン」11件の俯瞰調査（チャット記録）では、[[NETCASH-DUAL-CALC-1]]・
[[NETINCOME-DUAL-PIPELINE-1]]・[[RULE40-DEFINITION-MISMATCH-1]]が
この(a)の直接対象と確認された。

一方、以下の性質の重複には本規則は適用できず、別解が必要である:
- **(b) 単純なコード重複（DRY違反、定義の乖離なし）**: [[ERP-DUAL-
  CALC-1]]・[[Q4-IMPLIED-CALC-TRIPLICATION-1]]は、複数箇所に
  **同一の**計算式が複製されているだけで、命名・定義の乖離は発生して
  いない。対応は通常のリファクタリング（共通関数への集約）であり、
  命名規則の適用対象外。
- **(d) 外部データ取得（fetcher層）の重複**: [[FRED-HYSPREAD-TRIPLE-
  FETCH-1]]・[[SP500-GSPC-MULTI-FETCH-1]]・[[SEC-SUBMISSIONS-DUAL-
  FETCH-1]]は、同一の外部API呼び出しが複数箇所で独立実行される問題で
  あり、出力フィールドの命名規則ではなくfetcher層の統合設計
  （`INPUT_DATA_TOBE.md`のFRED統合層・yfinance統合層等）で解決する
  領域。

新規の重複パターンを発見した際は、まず(a)〜(d)のどれに該当するかを
判別してから、本ドキュメントの適用要否を判断すること。

**注記（2026-08-13、重複計算パターン棚卸し調査で発見）**: 上記「11件の
俯瞰調査」の原資料（チャットログ）は現存ファイルとして確認できず、
本ドキュメントが名前を挙げる(a)3件・(b)2件・(d)3件の計8件に加え、
`TO_BE.md`⑯群の`[[SEGMENT-FETCHER-DUPLICATE-ORPHAN-1]]`（完了済み）・
BACKLOG.mdの`[[MOAT-CATALOG-DUP-1]]`（ドキュメント重複カタログ）の
2件を合わせた**計10件までしか実在を確認できていない**。11件目の内容は
不明（実害なし、ドキュメント上の記録ギャップとして記録するのみ。
該当するBACKLOG項目を発見した場合はこの注記に追記すること）。
...
直接参照）。規則1はそもそも「データソースが異なる場合」の識別を
目的とするため、統一後は両者が同一ソースを参照するようになり、
接尾辞で区別する必要自体が消滅したとも解釈できる。是正案の例示は
将来的に実態へ合わせて更新することが望ましい（`[[NAMING-
CONVENTIONS-APPLY-1]]`参照）。
