...
Software_Systemグループ分割は完了。残るLITE/SITMカテゴリ欠落・
IOT等判定保留5銘柄・暫定判定精度の3課題は再評価待ち。固定比率設計の
限界・EBIT(1-t)→純利益変換ロジック不在の2課題は2026-07-15
...
へ統合済み（詳細は同エントリ参照）。このエントリには残課題1（LITE/SITM型
カテゴリ欠如）・残課題4（IOT等判定保留）・残課題5（暫定判定精度78%）のみ
残置する。
