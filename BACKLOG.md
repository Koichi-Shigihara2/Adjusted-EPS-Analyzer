...
### [FCF-OUTLIER-1] FCF 外れ値の誤除外（DOCN/LITE/VST）
**優先度:** 高
**分類:** バグ / EPS Analyzer / FCF outlier 除外ロジック

#### 症状
外部AIレビュー（78銘柄・2026-06-15）で DOCN・LITE・VST の FCF が
外れ値として誤除外されている疑い。正常な FCF 改善トレンドを持つ四半期が
スパイクとして除外されると、base_fcf が過小評価される。

#### 根本原因（仮説）
現行の FCF 外れ値除外は IQR ベースの固定閾値を使用しており、
FCF が急改善トレンドにある銘柄（赤字→黒字転換後）では
改善後の正常値が「外れ値」と誤判定される。

#### 修正方針（未着手）
1. トレンド検出を追加: 時系列トレンドが有意な場合は外れ値除外を緩和
2. または directional filter: 前期比で正方向の変化は除外しない
3. 影響銘柄: DOCN・LITE・VST 少なくとも3件

#### 現状
- 外部AIレビューで指摘。実装未着手。
- 対象銘柄の pipeline 再実行で影響を確認後に着手
...
### [SCORE-1] RICE × VALUATION MATRIX
**優先度:** 中
**分類:** 設計課題 / TANUKI SCORE

#### 内容
全登録銘柄を対象に、RICE指標（投資効率）と乖離率（バリュエーション）を
2軸にしたマトリクス表示をTANUKI SCOREに追加する。
割安×高効率の銘柄を一覧で視覚的に把握できるようにする。

#### 表示案
- 横軸: 乖離率（割安 ← → 割高）
- 縦軸: RICEスコア（高効率 ↑ ↓ 低効率）
- 各銘柄をプロットし、第1象限（割安×高効率）を強調表示
- データソース: `latest.json` の `upside_percent` と `rice.score`（tickers.json経由で全銘柄並列fetch済み）
