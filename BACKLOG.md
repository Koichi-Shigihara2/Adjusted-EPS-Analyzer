...
### 【フロントエンド表示課題】（2026-06-18 監査）

#### HOME画面
- [HOME-ANIM-1] LIVEドットのパルスアニメーション混在: パルスしているものとしていないものが混在している可能性。仕様を統一するか意図的な差異か確認が必要。
- [HOME-COLOR-1] ツールカードのテーマカラー近似: HYPE COREとSTONKS SILO等、近似色のカードがあり一瞬での識別が難しい。配色の見直しを検討する。

#### TANUKI SCORE画面
- [TSCORE-DISP-1] `–` と `N/A` の表示統一: FOUR/BKNG/FCX/JOBY/ASTS等でEPS成長・Rule40欄が `–` 表示。N/Aと`–`が混在していないか確認し、表示ルールを統一する。
- [TSCORE-DISP-2] JOBY・ASTSのフェーズ欄空欄: PASS銘柄でフェーズが表示されないケースがある。表示ルールを確認し統一する。
- [TSCORE-DISP-3] RICEマトリクスY軸ラベルの可読性: 縦書きラベルが読みにくい。表示方法の改善を検討する。
- [TSCORE-DISP-4] 判定実績バックテストの常時折りたたみ: デフォルト展開にするか現状維持かを検討する。
