...
- DISCOVERは On a Journey への登録候補（未登録銘柄）を発掘する場所であり、TANUKI SCORE等の登録済み・分析済み銘柄から投資対象を比較選定する場所とは役割が異なる
- 「仕込みゾーン不足」への適切な対応は、DISCOVERへの誘導ではなく、登録済み・分析済み銘柄群（TANUKI SCORE等）の中から仕込みゾーンに合う銘柄を自動でピックアップして提案する機能に変更すること
- 設計検討が必要
...
### [PORT-LAYOUT-3] ポートフォリオ明細テーブルの横スクロール解消（PORTFOLIO画面）
**優先度:** 中
- 「ポートフォリオ明細（保有中銘柄・全ブローカー合算）」テーブルが横に長く、27インチモニタの画面半分表示では横スクロールが必要になる
- 列構成・幅を見直し、横スクロールなしで収まるようにする
...
### [PORT-DISP-3] 明細テーブルの乖離率列が見切れている（PORTFOLIO画面）
**優先度:** 低
- 理論株価列の隣にある乖離率と思われる列が `+` のみ表示され見切れている
- 列幅を調整する

### [PORT-DISP-4] HYPEMIX「危険」バッジの判定基準説明がない（PORTFOLIO画面）
**優先度:** 低
- 「危険」バッジが表示されるが判定基準の説明がない
- 注記または凡例を追加する
...
### [TAIL-DISP-1] 「テーゼ健全度」の数値・基準説明がない（TANUKI TAIL画面）
**優先度:** 低
- PLTR=72(⚠)、SOFI=42(✗)、TSLA=47(⚠)等のスコアが何点満点で何を意味するか説明がない
- 凡例または注記を追加する
