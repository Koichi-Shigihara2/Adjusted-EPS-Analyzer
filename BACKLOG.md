...
### [MACRODATA-BAMLH0A0HYM2-HISTORY-EXCEPTION-1] BAMLH0A0HYM2はFRED側の2026年4月からの新規提供制限により再取得不可能、旧05_events.csvからの一度限りの例外的移行が必要
**状態:** 未着手（設計確定、実装待ち）
**優先度:** 中
...
#### 内容
`common/macro_data/series/BAMLH0A0HYM2.json`が785件（`2023-08-14`〜
`2026-08-10`）のみで、旧`05_events.csv`側の同系列（HY Spread）が
`1996-12-31`まで遡って存在する（7,709件）という深度差の原因調査を
実施した結果、以下が判明した:

- `fredapi::get_series_info("BAMLH0A0HYM2")`で取得した系列メタデータの
  `observation_start`が`2023-08-14`であり、`notes`欄に**「Starting in
  April 2026, this series will only include 3 years of observations.
  For more data, go to the source.」**と明記されている
- `observation_start="1990-01-01"`を明示指定してAPIを呼び出しても、
  返ってくるデータは`2023-08-14`以降のみ（FRED側の提供範囲そのものが
  制限されている、取得側のバグではない）
- `fetcher.py::update_series()`の保存前検証（重複as_ofチェック・桁
  ジャンプチェック）は警告をログに記録するのみでレコードを除外しない
  ことをコード上確認済み。`macro_data_violations_log.json`でも
  BAMLH0A0HYM2の警告は0件であり、検証ロジックによる除外という仮説は
  成立しない
- 全24系列（FTSD除く）についてFRED公式`observation_start`を機械的に
  調査した結果、**BAMLH0A0HYM2のみ**が2026年に入ってからの新規制限に
  該当し、他23系列は制限なし（`SP500`のみ別種の既知・恒久的な10年制限
  〈S&P Dow Jones Indices社との契約、2016年から存在〉に該当するが対象外）
- `common/macro_data/series/`の全24系列（FTSD除く）で、保存済み最古
  `as_of`とFRED公式`observation_start`が完全一致しており、取得・保存側の
  取りこぼしはないことを確認済み

`[[PHASE2-MIGRATION-POLICY-DECIDED-1]]`で確定した移行方針の「例外」規定
（データ提供元が恒久的にAPI経由の提供範囲を制限しており再取得が技術的に
不可能な場合に限り、旧保存先データから一度限りの例外的移行を行う）に
該当する、現時点で唯一のケースである。
...
- 投入範囲: `2023-08-14`より前の日付のみを追加投入する（`2023-08-14`
  以降は既にFRED APIからのライブ取得分が存在するため、既存レコードは
  変更しない）
- provenance: 移行したレコードの`source_detail`には例外的移行である旨を
  明記し、`source="FRED"`固定のライブ取得分（`source_detail=
  "series=BAMLH0A0HYM2"`）と区別できるようにする（具体的な文言は実装時に
  確定）

#### 着手条件
なし。次回の実装依頼で着手可能（今回はドキュメント記録・設計確定のみ、
実装は行っていない）。
