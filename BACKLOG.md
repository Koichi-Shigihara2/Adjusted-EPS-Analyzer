...
### [TAIL-COVERAGE-POLICY-UNDECIDED-1] TAIL satellite 7銘柄のRSS監視・レビュー自動生成対象範囲が方針未決定（`[[TAIL-SATELLITE-POSITION-MONITORING-GAP-1]]`の方針決定差し戻しにより再登録）
**優先度:** 中（データ破損ではなく運用上の欠落だが、保有ポジションへの
実カバレッジ欠落であり放置理由もない）
**分類:** 運用ギャップ / TAIL自動化パイプライン / 方針判断待ち
**登録日:** 2026-08-19
**発見:** `[[TAIL-SATELLITE-POSITION-MONITORING-GAP-1]]`（BACKLOG_DONE.md）
で一度「対応方針(b)＝core限定維持」が採用されたが、その判断がユーザーに
提示した対象範囲の誤認（1銘柄と説明→実際は7銘柄）に基づいていたため
差し戻された（`CHAT_RULES.md`事例6参照）。本項目はその差し戻しを受けた
再登録であり、実測データを添えて改めてユーザーの方針判断を仰ぐための
項目。
...
自動生成の対象から除外されているのは実際には**CELH・APP・NVDA・ADBE・
SOUN・CRWV・APGEの7銘柄＝保有10銘柄中7銘柄**だった。ユーザーは誤った
規模認識のまま方針(b)（core限定維持）を選択しており、判断の前提自体が
誤っていた。実装済みの対応（除外の`[SKIP]`ログ明示化、監視漏れ検出
`CHECK-37`）は方針の内容に依存しないため、そのまま有効。**方針
そのもの（satellite 7銘柄を今後も監視対象外のままにするか、対象に
含めるか）のみが未決定に戻っている。**

#### 実測結果（2026-08-19②、保有10銘柄すべて）
...
は3銘柄とも2026年5月時点までしか反映されておらず〈PLTR:2026-05-05、
SOFI:2026-05-07、TSLA:2026-04-23〉、CIが実行時に取得する最新データより
古い。これはローカルキャッシュの更新頻度に起因する既知の乖離であり、
本項目のcore/satellite比較そのものには影響しない）

#### Step 1-3: satellite銘柄は一度でも自動レビューされたことがあるか
`git log -p`で`review_queue.json`の全履歴を検索した結果、satellite
7銘柄のいずれかがキューに投入された記録（`"ticker": "XXX"`行）は
**全履歴を通じて0件**。`git log --all --diff-filter=A`で
`docs/portfolio/tail/data/reviews/`配下のファイル追加履歴を検索した
結果、satellite 7銘柄のレビューファイルが過去に追加されたことも
**0件**（過去に生成され後で削除された痕跡も含めて確認、該当なし）。
**したがって「過去には自動生成されていたが、ある時点から止まった」の
ではなく、7銘柄とも最初から一度も自動生成されたことがない。**

#### Step 1-4: 監視が及んでいない期間はいつからか
- `edgar_rss_monitor.py`は2026-06-06のコミット（`7c24c2e29`）で新規
  作成され、`get_core_tickers()`によるcore限定ロジックはこの最初の
  バージョンから既に存在していた（後から追加されたものではない）
- CELH/APP/NVDA/ADBE/SOUN/CRWVの各`thesis.json`は2026-06-05（
  `edgar_rss_monitor.py`新設の前日）に登録され、`type: "satellite"`は
  登録時点の値から一度も変更されていない（`git log -p`で全履歴を
  確認、`+"type"`行が各ファイル1回のみ＝初回追加時のみ）
- APGEの`thesis.json`は2026-07-02に登録され、同様に`type: "satellite"`
  は登録時点から一貫して変更なし
- **7銘柄いずれも、RSS監視の仕組みが存在するようになって以降（または
  各銘柄がTAILに登録されて以降のどちらか遅い方）、一度も監視対象に
  なったことがない**

#### 着手条件
ユーザーに、satellite 7銘柄（実測結果を添付した上での実数）を今後も
RSS監視・レビュー自動生成の対象外のままにするか、対象に含めるかの
投資方針判断を確認してから着手すること。

#### 関連
- `[[TAIL-SATELLITE-POSITION-MONITORING-GAP-1]]`（BACKLOG_DONE.md、
  本項目の元となった項目。実装済み部分〈`[SKIP]`ログ明示化・
  `CHECK-37`〉はそちらに記録済みで有効）
