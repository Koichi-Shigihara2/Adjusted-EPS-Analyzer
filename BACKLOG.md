...
### [DATA-FRESHNESS-MONITORING-FUTURE-IDEA-1] 各データパイプラインの「更新が一定時刻までに完了したか」を監視・通知する仕組みの構想（アイデア段階、未着手）

**優先度:** 低（実害は現状未確認。GitHub Actions基盤側の一時的な障害は
自然に収束するため、恒久的な問題ではない）
**分類:** 運用・監視 / GitHub Actions
**登録日:** 2026-08-29
**発見:** `[[MARKET-DATA-SCHEDULE-7AM-JST-1]]`（BACKLOG_DONE.md）の
異常遅延調査時。2026-08-26〜28にGitHub Actions基盤側の複数の
公式インシデント（githubstatus.com確認済み、詳細はBACKLOG_DONE.md
参照）により、`Market_Data_Daily_Update`等の複数ワークフローが
本来の起動予定時刻から3〜8時間遅延したことを実測確認した。

#### 内容
今回の遅延はGitHub Actions基盤側の一時的な障害が原因と判明しており、
リポジトリ側のコード・cron設定に恒久的な修正を加える余地はない
（起動待ち自体の遅延であり、ジョブ内の各ステップの実行時間は
遅延の有無に関わらず一定〈Market_Data_Daily_Updateは約3分〉だったと
実測確認済み）。

一方で、この種の基盤側障害は今後も再発しうる。現状は「Koichiさんが
実際に画面を見て初めてデータが古いことに気づく」以外に検知手段が
ない。以下のような監視・通知の仕組みがあれば、障害発生時に早期に
気づける可能性がある（あくまでアイデア段階、設計・実装コストの
見積もりは未実施）:

- 例1: 各ワークフローの完了後に`latest.json`等の`generated_at`/
  `calculation_date`を検査し、想定より古い場合はDiscord通知する
  軽量チェッカーを新設する
- 例2: GitHub Actions自体のワークフロー実行失敗通知（既存の
  Discord Webhook連携）に加え、「一定時刻までに完了イベントが
  観測されない」ことそのものを検知する仕組み（cronベースの
  ウォッチドッグ等）を別途設ける
- 例3: 単純にKoichiさんが気になったタイミングで手動確認する運用を
  継続し、仕組み化は見送る（現状維持案）

#### 対応方針（未確定）
上記いずれの方向性が適切か、実装コスト・監視対象の範囲（全ワーク
フロー対象か主要チェーンのみか）を含め、着手前に設計判断が必要。

#### 着手条件
なし。優先度低のため急ぎ不要。着手する場合はKoichiさんと方向性を
相談してから始めること。

#### 追加事例（2026-08-30、SEC_Data_Update障害調査時）
`SEC_Data_Update`ワークフロー（週次cron）が2026-08-16・08-23の2回連続で
「Consistency Check Gate」ステップで失敗し、`Commit and push changes`
ステップに到達できず、約3週間分（2026-08-09以降）のSEC EDGARデータ
更新が本番に反映されないまま放置されていたことが判明した（原因：
CIの依存パッケージインストールが`pip install requests`のみで
`pandas`/`yfinance`を含んでおらず、`report_consistency_check.py`が
インポート時エラーで即失敗していた。原因を特定しワークフローYAMLを
修正済み）。この件はKoichiさんが定期的に気になって確認するまで
誰も気づかなかった実例であり、本アイデアが想定する「検知手段が
ない」状態の実害が具体的に発生したことを裏付ける追加証拠である。
優先度・着手可否の判断自体は変更せず、Koichiさんへの報告時に
この実例を提示する。
