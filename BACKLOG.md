...
### [RECESSION-SCORE-TRIPLE-CALC-1] RECESSION RISK SCOREの3計算式併存・フェーズ境界25/30の3箇所不一致 — ①②は解消済み、③（2計算式併存の設計不透明性）のみ残存
**優先度:** 中（2026-08-22、高から見直し。理由は下記「訂正記録」参照）
...
### ✅ [MACRO-PULSE-ZONE-25-STALE-1] RECESSION RISK SCOREのフェーズ境界表示（ゲージバー・チャート）とAI解説プロンプトが「25」を使用、実閾値「30」と不一致 — 5箇所を30に統一して解消

**状態:** 完了（2026-08-21③。`docs/market-monitor/macro-pulse/index.html`
のゲージバー・ゾーン区切り線とゾーン目盛りラベル、チャート背景
`zoneMarkAreas`、チャート閾値ライン＋ラベルの計4箇所と、
`src/market/macro_pulse/05_main.py`のAI解説プロンプト文言1箇所、
合計5箇所の境界値「25」を「30」へ置換。フェーズ判定ロジック本体
（JS `renderPhaseGauge()`等の`score<30`分岐・Python
`if score < 30:`分岐）は元々正しく、表示・プロンプト文言側が
...
「踊り場」を切り替えていた（JS・Python共通）が、以下5箇所は境界値
「25」のまま未追従だった:
...
### [MACRO-PULSE-3M-FORECAST-SNAPSHOT-MISMATCH-1] 「3ヶ月先のマクロ予測スコア」（ライブ再計算）と「AIウィークリーコメンタリー」（週1回の凍結スナップショット）のスコアが一致しない
...
（`0〜25 拡張局面`・`25〜52 踊り場`）を発見した。同エントリのタイトル
「5箇所を30に統一して解消」は不正確になっている。今回は本項目の
スコープ外のため修正・BACKLOG追記のいずれも行わず、報告のみに留める。

#### 対応方針（未確定・次回セッションで判断、実装は投資判断に関わる
可能性があるため報告後に判断）
- 案①: 「AIウィークリーコメンタリー」側に「このスコアは
  {analysis_date}時点のスナップショットです」等の鮮度注記を追加し、
  ライブゲージと異なるタイミングの値であることを画面上に明示する
- 案②: 「3ヶ月先のマクロ予測」バナー文言を、実態（現在の先行指標
  合成スコア）に即した表現に修正する（例:「先行指標統合スコア（現在
  値）」等、時間シフト計算をしていないことが伝わる文言へ）
- 案③: AIウィークリーコメンタリーの生成頻度を週1回から日次に上げる
  （コスト増・Grok API呼び出し頻度増を伴う）
- 案④: 現状維持し、両者は「性質の異なる別指標」として画面上に明示する
  のみに留める
