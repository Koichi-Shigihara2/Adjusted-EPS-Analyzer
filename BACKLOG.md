...
#### 根拠の強化（2026-08-16、案2 Step C再検証）
`adjustments.py:634-636`の`(値 or 0.0)`パターンは現在も未修正で
あることを確認。加えて、`moat_result.moat_score`は表示専用の副次指標
ではなく、`moat_result.phase1_years`（高成長期間の年数）を経由して
`calculate_two_stage_dcf()`に**メインIV計算の入力として直結**して
いることをコード追跡で確認した（`core_calculator.py:433/594/635`、
いずれもメインIV算出経路の`_res_rm`計算に使われる`high_growth_years`
引数）。表示専用の副次指標ではなく、最大56/100銘柄でメインIVを系統的に
押し下げうる構造であるため、登録時より根拠が強い。

同種のパターンは`[[STONKS-SILO-COGS-DEAD-FALLBACK-1]]`
（RXRX 2021年、2026-07-30対応済み、BACKLOG_DONE.md参照）で一度発見
されている。同項目の「副次的発見・修正」欄に記載の通り、Pythonの
`0`がfalsy値であるためにor演算子ベースの分岐が正当なゼロ値を欠損値と
誤認する（またはその逆）というfalsy-zeroパターンであり、本項目の
`(値 or 0.0)`（Noneも正当な0.0も同じ扱いにしてしまう）と同型の欠陥
クラスに属する。
