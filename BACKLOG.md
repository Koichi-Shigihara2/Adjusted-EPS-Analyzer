...
#### 実害範囲（growth_sanity確認済み・2026-07-15）
現時点でgrowth_sanity.calc_revenue_cagr()への実害はなし
（AVGO/DELL/CAKE/ELFいずれも、破損している年度がCAGR参照窓
［年[-4]・年[-6]のみ参照］の外にあるため）。現行latest.jsonの
rev_cagr_3yr/5yr/verdictは4銘柄とも変化なしと確認済み
（AVGO=PLAUSIBLE、DELL=AGGRESSIVE、CAKE=PLAUSIBLE、ELF=REVIEW）。
TANUKI VALUATIONのFCF計算窓（直近5年）にも直接の影響なし。

#### duration filter対応の試行結果（実装は復元・未コミット、2026-07-15）
「四半期スタブを年次バケツから除外する」最小修正を試行したが、
以下の理由で目的（誤った値→値なしの明示）を達成できないと判明：

| 銘柄 | 挙動 |
|---|---|
| AVGO | 2018年はNone化（成功）。2019年→$20,848M、2020年→$22,597Mに変化（いずれも隣接する別の実年度の真の値がラベル誤りのまま横滑り） |
| DELL | 2017年はNone化。2018年→$61,642M（実は真のFY2017値）、2019年→$79,040M（実は真のFY2018値）に変化。かつ真のFY2019値（$90,621M）自体が、修正前後を問わずどこにも存在しない（別の真の年度データ$92,154Mとend_yearバケツで衝突しtie-break規則で敗れて消失。duration filterでは対処不可能な別種の衝突） |
| CAKE | 2011年・2016年はNone化。2012年・2017年・2018年は隣接年度の値に置き換わりラベルは誤ったまま |
| ELF | 5年分（2015-2019）全てが変化したが1年もNone化せず、全て隣接年度の真の値に置き換わっただけでラベルずれは解消せず |
| RCAT | 変化なし（別原因〈再表示の不整合〉であり本件の対象外と別途確認済み） |

根本原因はduration filterで対処できる範囲を超えている：
①複数年度にわたる系統的な年度ズレ（1年単位の穴埋めでは追いつかない）、
②真の値同士のend_yearバケツ衝突（DELLのFY2019消失）という、
duration（期間長）とは無関係な別種のtie-break欠陥、の2点が絡んでいる。

#### 対応方針（未確定・要根本修正の設計）
`determine_fiscal_year()`自体の52/53週会計年度対応（ARCH-DATA-1-FYで
統一済みの共有関数、parser.py/extract_key_facts.py/aggregate_annualの
3箇所が参照）が必要と考えられるが、全100銘柄のfiscal_year判定に
影響する共有関数のため回帰リスクが高い。着手前に、他に52/53週会計
年度を採用する銘柄が存在するか確認が必要（AVGO/DELL/CAKE/ELF以外にも
存在する可能性）。

**追記（2026-07-15・記録漏れの追加登録）**: CAKEの`annual_2022.json`が
存在しない（2021年の次が2023年に飛んでいる）ことをgrowth_sanity実害
確認時に発見済み。本件と同一の年度バケツ衝突により候補が完全に消失
した結果である可能性があるが未検証。根本修正の設計時に合わせて原因を
確認すること。

**注記**: 残課題③（[[ARCH-DATA-1]]参照）は着手前調査の結果、汎用
パターンカタログの新設は精度未検証のリスクから見送り、revenue系タグ
競合の実データ検知スクリプトへスコープを縮小して完了した。当初本
エントリで想定していた「ARCH-DATA-1のパターンカタログへの蓄積」は
発生していない。本52/53週会計年度パターンは、本エントリ単独で記録・
追跡する。

#### 着手条件
なし（実害は限定的〈CAGR窓外〉のため緊急性は低いが、根本原因が
...
#### 追記（IOT _build_period_data追加調査完了・根本原因＝回帰バグと判明・2026-07-15 同日3回目）

**決定的発見：ARCH-DATA-1-FYコミット（2026-06-25, ab792d38b）による回帰**
IOTの「annual_2024.json/annual_2025.jsonの完全重複＋真のFY2025値の
データセット全体からの消失」現象を追跡した結果、`_build_period_data`
自体には重複・上書きロジックが一切ない（単一辞書ルックアップのみの
純粋関数）ことを確認。原因はdetermine_fiscal_year()導入コミット
（ARCH-DATA-1: 年度判定共通関数化、2026-06-25）自体にあった：

- 旧ロジック（〜06-24）: `end_year = int(end_date[:4])`（単純な暦年抽出）
- 新ロジック（06-25〜）: `determine_fiscal_year(end_dt, fiscal_end_month)`
  （月比較）

IOTの真のFY2024（end=2024-02-03）は、旧ロジックでは正しく「2024」に
分類されていたが、新ロジックでは月(2)>fiscal_end_month(1)により「2025」
に計算されるようになり、「2024」という年度キー自体が新ロジック下では
恒久的に生成されなくなった。`save_parsed_data()`（parser.py:611-626）
には古い年度ファイルを削除・無効化する処理が一切ないため、06-24時点で
生成されたannual_2024.json（当時は正しい内容）がそのまま「化石ファイル」
として凍結され、あたかも今も正しく生成されているかのように見えていた。

**影響範囲の再訂正（重要）**
機械的な全銘柄スキャンにより、この「化石ファイル」現象はIOT固有ではなく、
**非12月決算かつ四半期実績のfp="FY"混入がない52/53週企業（IOT・AVGO・
MRVL）に共通する現象**と確認：
- 12月決算企業（CAKE/CDNS/JNJ/TDY）の衝突は**このコミットと無関係**
  （month>12は数学的に常に偽のため新旧ロジックで結果が同一。これらは
  コミット前から存在していた別種の欠陥）
- 非12月決算企業のうちDELL/ADBE/LITEは、10-K内に四半期実績がfp="FY"の
  まま混入する慣行があるため、空いたバケツが90日スタブで継続的に
  埋め戻される（＝ファイルは更新され続けるが誤った値になる＝
  「誤配置」として現れる）
- IOT/AVGO/MRVLにはそのようなスタブ混入がなく、空いたバケツを埋める
  候補が皆無なため、コミット前の最後の正しい状態がそのまま凍結される
  （＝「化石ファイル」として現れる）

いずれも根本原因はFY52WEEK-BUCKET-MISPLACE-1と完全に同一
（determine_fiscal_year()の月のみ判定・日を見ない設計）であり、
見た目の違いは「空いたバケツを埋める代替値の有無」という副次条件のみ。

**修正スコープへの結論**
1. 独立タスクへの分離は不要。determine_fiscal_year()を日付ベース判定に
   改修すれば、化石ファイル現象・誤配置現象ともに自動的に解消される
   見通し。
2. **ただし save_parsed_data() に古い年度ファイルの削除ロジックがない
   ため、ロジック修正だけでは既存の化石ファイル・誤配置ファイルは
   自動是正されない。** 根本修正の実装計画には、対象9銘柄
   （IOT/AVGO/MRVL/DELL/JNJ/ADBE/CDNS/TDY/LITE）の
   `common/sec_data/data/{TICKER}/annual_*.json`の明示的な全再生成
   手順を含める必要がある。
