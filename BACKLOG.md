...
### 具体的な3課題
1. **基準FCFに2年平均を使用** → 直近の悪化を半分しか織り込まない。減少トレンドがある場合は直近値（または保守的推定値）を基準にすべき
2. **推奨成長率とDCF計算値の乖離** → HypePhase補正後の推奨成長率がレポートに表示されるが、DCF計算は別の成長率で実行されており内部矛盾が生じる
3. **FCFマージン悪化が成長率に未反映** → 売上成長率をそのままFCF成長率に使用しているが、FCFマージンが低下していれば売上成長≠FCF成長

### 追加観点
- FCF減少が「先行投資（一時的）」か「構造的悪化」かの区別をDCFシナリオに反映する仕組みが未整備

### 改善案（要設計）
- FCFトレンドがCAGR < 0の場合、基準FCFを直近値に自動切替
- 推奨成長率をDCF計算の入力として優先使用（または整合性チェックを追加）
- FCFマージントレンドをBEARシナリオの成長率上限として活用
...
現状の改善余地評価と潜在ROEを算出する機能をTANUKI VALUATIONに追加する。
