...
- **型分類の所見**: [[ANOMALY-PATTERN-CATALOG-1]]の型A（候補集合＋
  ...
  新パターンと判断し、**[[ANOMALY-PATTERN-CATALOG-1]]の型C（資産クラス
  変化・当年度未タグ化型）としてカタログ本体へ追加登録済み**（2026-07-19、
  詳細な症状・根本原因・対応方法は同エントリの型C定義を参照）。他に
  同型が疑われる銘柄は現時点で未確認（[[BS-FIELD-NONE-TRANSITION-
  DETECT-1]]参照：本件の発見経緯自体、系統的検知ではなく偶然の個別
  調査によるものだった）
...
一次情報（SEC EDGAR）で確認: RCAT（CIK 0000748268）は決算期を2回変更
している（①2019年 12月末→4月末、②2024-2025年 4月末→12月末）。②の
変更（元の決算期に戻る形）で、境界年2024に4月期12ヶ月の本人データ
（total_assets=$48,537,612）と、12月期8ヶ月スタブの非本人データが
両方computed_year=2024に衝突することを確認した。

現状は`_own_override_is_safe`の汎用accnベース判定（本人データ優先、
ARCH-DATA-1残課題④実装分含む）の副次効果で結果的に正しい値が採用
されているが、この衝突自体を検知・記録する仕組みは存在しない。理由:
衝突する2エントリの生XBRLのfyタグが元々異なる（本人データ側fy=2024、
非本人データ側fy=2025）ため、CHECK-22（同一fyタグへの複数end_date競合）
にもCHECK-23（fyタグ裏取り、is_own_data=True限定）にも該当しない。

105銘柄全体を「support≧2の実質的クラスタが2つ以上・循環距離30日超」で
機械的スキャンした結果、該当候補はRCAT・ELF・MSCI・NOW(ノイズ)の4銘柄。
うち実際に境界年衝突が発生しているのはRCATのみ（ELF/MSCIは変更後の
最初の年度に競合エントリがなく衝突なし。NOWは2013年10-Kの単発参考開示
でノイズと判定）。

#### 実害範囲
現状ゼロ（RCAT 1銘柄のみ、かつ現在の値は正しい）。ただし
`_own_override_is_safe`側の将来の変更や、RCATのaccn欠落等の
データ側の変化で無防備に崩れうる潜在リスクとして残る。

#### 対応方針案（未実装・次回セッションで判断）
クラスタリングで2つ以上の有効クラスタ（support≧2・循環距離30日超）を
検出した銘柄について、変更前後の境界年で本人データ同士の衝突有無を
明示的にログ化する専用チェック（新規WARN、例: WARN-24）を追加する案。
...
- 対応方法: 単一タグでの完全解消は不可能なことが多い。選択肢は
  ...
  ③ 当面はNoneのまま許容し、Net Debt計算等の下流への影響を
     個別確認する
- 実例: NVDA（short_term_investments、2026-07-19発見・
  対応方針未確定）
