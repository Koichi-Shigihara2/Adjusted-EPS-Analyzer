...
### [EPIC-LEGEND-1] 指標説明・凡例コンポーネントの共通化
**優先度:** 最高
**統合元:** HYPE-DISP-3/4/5, DISCOVER-DISP-1/2, PORT-DISP-4, TAIL-DISP-1,
MP-DISP-2/3/4/6/7, MACRO-DISP-2/3/4/5, SILO-DISP-1/2/4,
TSCORE-DUPONT-2/4, TVAL-FORMULA-1（計18件超）

#### 問題の本質
「この数値・バッジ・表記は何を意味するか」という説明欠如が、ほぼ全システムの
個別ページで同時多発している。原因は **画面ごとに凡例をベタ書きしているため、
新指標を追加するたびに説明追加を忘れる** という構造にある。1件ずつ直しても
次の新機能で同じ抜け漏れが再発する。

#### 対応方針
- `docs/common/` に「インフォツールチップ共通コンポーネント」（JS）を1つ作成
  - `<span data-info="key">` のようなマークアップ + 用語辞書JSON
    （`docs/common/glossary.json` 等）を1箇所に集約
  - 用語辞書はシステム横断で再利用（例: 「DCF_Reliability=LOW」の説明は
    TANUKI VALUATION/TANUKI SCORE両方で使う）
- 既存の個別指摘18件は、この共通コンポーネント実装後に「用語辞書へのエントリ追加」
  という機械的作業に縮退する（1件あたりの工数が激減する）
- 新規指標を追加する際は CLAUDE_CODE_START.md の「新規銘柄属性を追加した場合の
  必須対応」に準じ、「ユーザー向け数値・バッジを追加した場合は glossary.json に
...
#### 残課題（個別収束分）
B分類4銘柄（BSY/ELF/ESTC/FICO）: Python BUY vs JS WATCH。
JS側のBUY条件にtiming>=50ゲートあり・Python側にはなし。
**設計判断が必要**: Pythonにゲートを取り込むか、JS側のゲートを撤廃するか。
着手前に判断を仰ぐこと。
...
#### 着手条件
B分類4銘柄の設計判断（timingゲートの扱い）を先に決めてから、
一本化の設計に着手する。
...
- 根拠となるデータポイントをツールチップ等で確認可能にする（EPIC-LEGEND-1の
  共通コンポーネント完成後はそれを再利用する）
...
- 「確信度：高/中」の判定基準も不明（EPIC-LEGEND-1完成後はそちらで統一対応）
...
- 実装コードと全算式を照合して修正する
- ※表示コンポーネント自体はEPIC-LEGEND-1で統一されるが、本件は「内容の正しさ監査」
  であり別タスクとして残す
...
- 使い方の説明・凡例を追加するか、チャート自体の見せ方を再設計する（EPIC-LEGEND-1適用候補）
...
どちらも「個別バグ修正の繰り返しコスト」を生み出す根because源であるにも
