...
### [MARKETDATA-DAILY-UNADJUSTED-PRICE-DIVIDEND-DRIFT-1] daily/層がauto_adjust=False（未調整終値）で保存されており、配当・特別配当銘柄の移動平均系指標が体系的にズレる

**優先度:** 高（`common/market_data/`本番消費者切替〈着手順序4〉で
切替済みの複数ファイルに影響する可能性がある根本的な設計上の問題。
`[[MARKETDATA-LAYER-CONSTRUCTION-1]]`着手順序4-8＝本番消費者切替の
最後の1件の検証で偶然発見されたが、影響範囲は他の切替済み消費者にも
及ぶ）
**分類:** データ設計上の問題 / yfinance auto_adjust仕様の見落とし
**登録日:** 2026-08-11
**発見:** `[[MARKETDATA-LAYER-CONSTRUCTION-1]]`着手順序4-8
（hypecore.py本体切替）のStep3検証、104銘柄全数比較中（チャット記録、
2026-08-11）

#### 内容
`common/market_data/fetcher.py::_download_historical_bars()`は
`yf.download(symbols, ..., auto_adjust=False, ...)`で日次価格を取得し、
`daily/{SYMBOL}.json`へ**未調整終値（配当・分割で遡及調整されない生値）**
のまま保存している。一方、切替前のhypecore.py本体は
`t.history(start=start, auto_adjust=True)`で**調整済み終値**（配当支払い
時点より過去の価格を配当額分だけ遡及的に引き下げた、トータルリターン
ベースの連続系列）を使用していた。

この2つの流儀の違いが、配当（特に特別配当）を支払う銘柄の200日移動
平均乖離率（`ma200_dev`）に体系的なズレを生むことを、hypecore.py
104銘柄全数比較で発見した。8銘柄で乖離率が閾値（±1pt）を超えて不一致
となり、**全銘柄が直近200営業日以内に配当支払い実績を持つことを実測で
確認**した:
- `TASK`: 特別配当$3.65（2026-03-26）。乖離率 旧+4.52% → 新-20.27%
  （**24.8pt差、above_ma200判定が反転しうる規模**）
- `KO`/`ABBV`/`MO`/`PEP`/`PM`/`VZ`: いずれも高配当利回りの著名銘柄
  （四半期配当を継続的に支払っており、200日ウィンドウ内に3〜4回の
  配当が累積）
- `CIX`: 四半期配当$0.30を継続支払い

根本原因: `auto_adjust=False`で取得した生値は、配当支払い日をまたいで
`rolling(200).mean()`のような移動平均計算を行うと、配当額に相当する
「見せかけの下落」を配当支払い日に含んでしまい、ウィンドウ内の配当
累積額に比例して移動平均が実態より低く算出される（結果、直近終値の
乖離率が実態より過大に振れる）。

**影響範囲（本エントリ発見時点で確認・推定したもの）**:
- `hypecore.py`（本切替、着手順序4-8）: `ma50_dev`/`ma200_dev`が
  配当銘柄で系統的にズレる。`determine_stage()`のS3/S4判定はいずれも
  `ma200_dev`の閾値判定を含むため、配当銘柄でステージ誤判定が起こり
  うる
- `reader.get_ma_deviation()`を使う**他の切替済み消費者すべて**が
  同一の根本原因を共有する可能性が高い（未検証、要確認）:
  - `data_fetcher.py`（着手順序4-2）: ma200代数逆算に
    `get_ma_deviation(window=200)`を使用。配当株のDCF計算のma200値が
    影響を受けている可能性
  - `collect_and_send.py`（着手順序4-6）: `_get_sp500_ma_deviation()`
    が`^GSPC`（指数、配当込み再投資指数ではないためこの問題は
    軽微と推定されるが未検証）・`fetch_qqq_tech_data()`が`QQQ`
    （配当を支払うETF）に影響しうる
  - `breadth_calculator.py`（着手順序4-7）: `pct_above_50ma`/
    `pct_above_200ma`の算出が全S&P500銘柄（多数が配当銘柄）に及ぶため、
    集計値が体系的に歪んでいる可能性
- 影響の大きさは「配当利回り × ウィンドウ内配当回数」に比例するため、
  無配当・低配当銘柄（成長株中心）では実害は小さいと推定されるが、
  高配当銘柄・特別配当発生銘柄では無視できない規模になりうる

#### 対応方針（未定、要判断）
以下の選択肢が考えられるが、影響範囲の全容調査と合わせて判断が必要:
1. `_download_historical_bars()`の`auto_adjust`を`True`に変更し、
   `daily/`全体を再バックフィルする（最も根本的な解決だが、
   570銘柄×約1400日分の再取得・全消費者への影響再検証が必要な
   大規模作業）
2. `daily/`に調整済み・未調整の両方を保存する（ストレージ倍増、
   設計の複雑化）
3. 影響が実際に軽微と判断できる場合は既知の制約として文書化し、
   現状維持する
4. 移動平均系の計算が必要な消費者（hypecore.py・data_fetcher.py等）
   のみ個別に配当調整を後掛けする（実装コスト・一貫性の観点で
   非推奨）

#### 着手条件
なし。まず他の切替済み消費者（data_fetcher.py・collect_and_send.py・
breadth_calculator.py）への実害範囲を確認してから対応方針を確定する
必要がある。
