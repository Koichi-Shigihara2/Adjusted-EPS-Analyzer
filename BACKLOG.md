...
### [MATRIX-1] ROE_avg窓長の決定ロジック明文化
**優先度:** 低
**分類:** 設計課題 / TANUKI VALUATION (Matrix象限判定)

#### 背景
現状の`get_roe_avg_detail()`は「equity>0の全年」を平均対象とするため、
銘柄ごとのデータ可用年数によってROE_avg窓長が変動する（例: VST=7yr avg 10.5% vs 直近3yr avg 31.4%）。
窓長が異なると象限判定（閾値15%）の再現性が低く、外部レビューで「計算結果が一致しない」と誤指摘されやすい。

#### 対応案
- **(a) 全銘柄固定Nyr**: 例えば最新5yr固定。新規上場・データ不足銘柄は利用可能年数で補完。
- **(b) 動的採用+report表示**: 現行動作を維持しつつ`ROE_avg_window: 7yr`をreport.txtに明示。再現性問題を「可視化」で解決。

#### 推奨
(b)が低コスト。(a)は窓長変更が全銘柄のIV（Matrix象限経由）に波及するため要注意。

#### 関連
VST: ROE_avg(7yr)=10.5% vs 直近3yr≈31% → 窓長次第でQ2↔Q3象限が変わる（閾値15%が境界）
...
バグの大半（BUG-ROE-NI-1 / SEC-REV-FINTECH-1 / BUG-NETDEBT-5等）は
...
- BS項目はすべて同一決算期（同一 as-of 日）から取得する統一規則を敷く
  （現状は Cash が最新四半期・ST_Invest が年次のような項目別バラつきが
   期ズレバグ BUG-NETDEBT-5 を生んだ）
- 旧SECタグ・金融revenueタグ・非12月期等の吸収を正規化層に集約し、
  計算ロジックからデータ個性の処理を排除
- normalized JSON に不足フィールド（ShortTermInvestments 等）を補完
...
- 現状: 未実装
- 改善: 訴訟・規制調査・DEI関連・関税影響等の既知リスクをレポートに追記
