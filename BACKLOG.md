...
### [TANUKI-ROE-3] DuPont分解の改善（一過性NI/期間トレース/表示/極小売上除外）テスト追加
**優先度:** 中
**背景:** 2026-06-17にDuPontレビューで発見した4件の改善（一過性NI集中フラグ・
dupont_bs_period・表示バッジ・極小売上除外）を実装。テストは別途追加が必要として
今回は未実装（ユーザー指示によりスキップ）。
**残課題:**
- `tests/test_pipeline_logic.py` にDuPont計算のユニットテストを追加
  （除外条件3パターン・reliability=LOW判定・revenue_too_small判定）
- 売上閾値$10Mの妥当性再検証: QBTS（TTM Revenue=$12.1M）は僅差で除外対象外のまま。
  現状は net_margin=-2008%という極端値が表示され続けている（reliability=LOWにもならない、
  単四半期集中ではなく売上規模そのものが原因のため）。閾値を$15M前後に引き上げるか、
  別の判定軸（資産回転率の異常値等）を検討する
