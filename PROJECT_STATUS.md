...
| 分類C14件（`INPUT-C-001〜014`）の管理方法検討（`config/`外配置2件の是正、Portfolio二重保持の是正等） | **登録済み3件が全て実装完了（2026-08-15）**: `[[TAILKPI-CONFIG-LOCATION-1]]`（`INPUT-C-009`、`config/tail_kpi_map.json`へ移動完了・コミット`80890c711`）・`[[FCFCONFIG-LOCATION-1]]`（`INPUT-C-010`、`config/fcf_conversion_config.json`へ移動完了・コミット`493e8843a`）・`[[PORTFOLIO-CONFIG-DUP-1]]`（`INPUT-C-008`、`config/portfolio.json`を廃止し`docs/portfolio/data/portfolio.json`へ一本化・コミット`e97741f54`/`eaf3016cb`）。残り11件（`INPUT-C-001〜007`・`011〜014`）は未着手。なお`INPUT-C`番号を持たない関連課題`[[DISCOVER-CONFIG-DUAL-MGMT-1]]`（Discoverのconfig二重管理）は、Aグループ実装直前調査で`config/discover_config.json`に独立したPython消費者（`src/discover/collect.py`のDiscoverパイプライン本体・`common/sec_data/registration_validator.py`の登録監査）が存在することが判明し、`[[PORTFOLIO-CONFIG-DUP-1]]`と同型の「docs/側へ一本化・config/側廃止」という解決法がそのまま適用できないと判明。実装を停止し設計再検討中（`config/`を正・`docs/`を自動同期コピーとする非対称設計へ再修正予定、BACKLOG.md参照、未完了） |
