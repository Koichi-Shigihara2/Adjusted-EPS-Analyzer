...
一次データ層の総数を明記。フェーズ1のスコープを「分類A＋分類B（SEC EDGAR
取得の前提条件）」に絞り直し、分類Cはフェーズ3〈導出データ層検討〉側に
持ち越す旨を明記）
...
## 一次データ層の総数（`INPUT_DATA_TOBE.md`3分類、2026-07-23時点）

| 分類 | 件数 | ID範囲 | フェーズ1・2のスコープ内か |
|---|---|---|---|
| A. 一次データ本体 | 47件 | `INPUT-A-001`〜`047` | **対象**（一次データ層構築の主対象） |
...
## フェーズ1: 一次データ層の構築（分類A47件＋分類B3件が対象）
