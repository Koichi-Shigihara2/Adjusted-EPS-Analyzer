...
| 分類C14件（`INPUT-C-001〜014`）の管理方法検討（`config/`外配置2件の是正、Portfolio二重保持の是正等） | **進行中（2026-08-15更新）**。分類C14件のうち3件をBACKLOG登録済み、うち**2件が実装完了**: `[[TAILKPI-CONFIG-LOCATION-1]]`（`INPUT-C-009`、`config/tail_kpi_map.json`へ移動完了・コミット`80890c711`）・`[[FCFCONFIG-LOCATION-1]]`（`INPUT-C-010`、`config/fcf_conversion_config.json`へ移動完了・コミット`493e8843a`）。残り1件`[[PORTFOLIO-CONFIG-DUP-1]]`（`INPUT-C-008`）は**設計方針修正済み・実装未着手**（`docs/portfolio/data/portfolio.json`を唯一の正とする方針に確定、GitHub Pages配信制約により当初案「config/を唯一の正に」は撤回。詳細は`SYSTEM_MAP.md`「`config/`と`docs/`の配置原則」参照）。残り11件（`INPUT-C-001〜007`・`011〜014`）は未着手。なお`INPUT-C`番号を持たない関連課題`[[DISCOVER-CONFIG-DUAL-MGMT-1]]`（Discoverのconfig二重管理）も同一の2026-08-15フェーズ3合同設計調査で対応方針を修正済み・実装未着手（`[[PORTFOLIO-CONFIG-DUP-1]]`と同方向、docs/側への一本化） |
