...
### ✅ [TAIL-SATELLITE-POSITION-MONITORING-GAP-1] TAIL保有銘柄APGEが自動レビュー生成パイプラインから完全に漏れている（satellite種別＋tanuki=falseの二重要因）— 除外の明示化とCHECK-37は完了、対象範囲の方針判断は差し戻し
**状態:** 部分完了（**実装部分のみ**完了。除外の沈黙を解消する
`[SKIP]`ログ明示化と監視漏れ検出`CHECK-37`は有効なまま存置する。
**方針(b)＝core限定維持の採用決定は誤った前提に基づいており差し戻した
（下記「方針決定の差し戻し」参照）。方針そのものは未決定に戻り、
`[[TAIL-COVERAGE-POLICY-UNDECIDED-1]]`として改めてBACKLOG.mdへ登録した**
**優先度:** 中（データ破損ではなく運用上の欠落だが、保有ポジションへの
実カバレッジ欠落であり放置理由もない）
**分類:** 運用ギャップ / TAIL自動化パイプライン / バグ
**登録日:** 2026-08-19
**完了日:** 2026-08-19（実装部分のみ。方針判断は`[[TAIL-COVERAGE-
POLICY-UNDECIDED-1]]`へ差し戻し・再登録、2026-08-19②）
...
（BACKLOG.md）および`CHAT_RULES.md`事例6を参照。

**「APGEのレビューが0件である状態は今後も継続する」という記述は誤りと
して削除する。** 方針は未決定に戻ったため、この状態が今後も継続するか
どうかは確定していない（保留）。将来この節を見た人が「仕様として確定
した」と誤認しないよう、ここに明記する。
...
- `SYSTEM_MAP.md`「TANUKI TAIL」節に監視方針を記録（2026-08-19追記。
  方針差し戻しにより一部記述が古くなっている可能性、参照時は
  `[[TAIL-COVERAGE-POLICY-UNDECIDED-1]]`の状態を優先すること）
- `[[TAIL-COVERAGE-POLICY-UNDECIDED-1]]`（BACKLOG.md、2026-08-19②新規。
  本項目の方針判断を差し戻して再登録した未決定項目。satellite 7銘柄の
  監視実態の実測データを含む）
