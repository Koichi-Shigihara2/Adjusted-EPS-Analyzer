...
### ✅ [DATA-JUMP-CHECK-GENERALIZE-1] Revenue以外のフィールドへの段差型検知の展開（売上総利益・CapExの2項目に限定）
**状態:** ✅完了
**優先度:** 未定→中（着手時に確定）
**分類:** アーキテクチャ / 品質管理
**登録日:** 2026-07-12
...
Revenueのみに限定していた（現状Revenue専用のハードコードで、対象フィールド
をパス引数化する改修が実質的な新規設計として意図的に対象外だったため）。
CapEx・NetIncome・GrossProfit・SBC等の他フィールドにも段差型の前年比急変
（タグ切替・タグ取得ミスによる不連続）が理論上起こりうる（LLYのCapExは
TTM-QUARTERS-CHECK-1の副産物として偶然発見されたに過ぎない）。

#### 対応内容
1. `common/screening/dcf_validity_checker.py::check_c_data_jump()`を
   `section`・`field`・`down_ratio`引数でパラメータ化した。Revenue呼び出し元
   （CHECK-21・WARN-21）はデフォルト引数（`section="pl", field="revenue",
   jump_ratio=2.0, down_ratio=None→1/jump_ratio=0.5`）で従来と完全に同一の
   挙動を維持する（回帰テストで確認済み）
2. `report_consistency_check.py`に新規チェックを2件追加:
   - **CHECK-44/WARN-44（売上総利益）**: `pl.gross_profit`、上振れ5.0倍/
     下振れ0.2倍
   - **CHECK-45/WARN-45（CapEx）**: `cf.capital_expenditure`、上振れ8.0倍/
     下振れ0.15倍（上振れの逆数0.125とは異なる非対称な閾値のため、
     `down_ratio`引数で明示指定する設計とした）
   いずれもPhase 2b-2の教訓「2.0倍/0.5倍という閾値はNGには誤検知率が
   高すぎる」を踏まえ、NGではなくWARNとした
3. 純利益・SBCは今回のスコープから除外（下記「スコープを2項目に限定した
   経緯」参照）し、[[DATA-JUMP-CHECK-NETINCOME-SBC-1]]として低優先度で
   別途新規登録した

#### スコープを売上総利益・CapExの2項目に限定した経緯（純利益・SBC除外の理由）
tanuki=true全100銘柄・直近6年のYoY比率分布を実測した結果、純利益・SBCは
比率方式が本質的に機能しないことを確認した：
- **純利益**: 477件中53件が符号反転（黒字↔赤字）。符号反転を跨ぐ比率は
  数学的に意味を持たない（例: LITE 2025→2026で比率-267.76倍という値は
  「267倍悪化」を意味しない）
- **SBC**: 実測でZETA（2020→2021、$105K→$259.16M、倍率**2468.18倍**）
  という正当な急増（上場後の株式報酬制度導入）が存在し、比率の上限を
  無意味化する

一方、売上総利益・CapExは実測分布（p1〜p99）が閾値と整合的で、比率方式
がそのまま機能することを確認できたため、この2項目のみ先行実装した。

#### 実測分布と閾値設定の根拠（依頼書記載値との差異について）
着手時にtanuki=true全100銘柄・直近6年で独自に実測したところ、依頼書
記載の参考値（売上総利益p99=4.55・p1=-0.49、CapEx p99=6.83・p1=0.25）
とは異なる分布が得られた（売上総利益p99=7.73・p1=-0.24、CapEx
p99=8.32・p1=0.14）。データ再パース等による差異と推測されるが、
依頼書が明示的に「着手時に実データで再確認・調整すること」としていた
ため、独自実測に基づき閾値の妥当性を再確認した。結果として提示された
閾値（売上総利益5.0/0.2、CapEx8.0/0.15）はいずれも自己実測のp95〜p99
近辺に位置し、「上位数%の極端な変化のみを拾う」という設計意図に
引き続き整合していたため、そのまま採用した。

#### 検証結果（実データバックテスト、2026-09-06）
tanuki=true全100銘柄・直近6年でバックテストした結果:
- **WARN-44（売上総利益）**: 16件（対象: ASTS/CRWV/IONQ/JOBY/KULR/ONDS/
  RCAT/RXRXの8銘柄）
- **WARN-45（CapEx）**: 10件（対象: ALAB/BBAI/FLYW/KULR/ONDS/RCATの6銘柄）
- いずれも小型株・上場間もない銘柄でゼロ近傍からの立ち上がり・設備投資
  急増が主因であり、閾値が緩すぎて何も検知しない、または厳しすぎて
  大量誤検知が出ている状態ではないことを確認

**LLYのCapExでの検証について（依頼書の前提と異なる結果）**: 依頼書は
「LLYのCapEx（TTM-QUARTERS-CHECK-1の副産物として発見）で実際に検知
されることを確認する」としていたが、実データを確認したところLLYの
現在のCapEx系列（2007-2025年、全19年）はいずれの隣接年比も8.0倍を
超えず・0.15倍を下回らないことを確認した。原因を調査したところ、
[[LLY-CAPEX-STALE-1]]（2026-07-12完了・2026-08-05訂正記録）で判明した
実際の不具合は「YoY比率の異常な段差」ではなく「`capital_expenditure`
フィールド自体が全19年度にわたり存在しなかった（Noneのまま）」という
異なる種類の欠損だった（修正時に全19年分が同時に新規追加されたため、
現在のannual_YYYY.jsonには段差の痕跡が残っていない）。`check_c_data_jump()`
は`v0 is None`のペアをスキップする設計のため、この種の「フィールド
まるごと欠損」はそもそも検知対象外であり、本チェックの対象外の問題
だったと判明した。この経緯をBACKLOG_DONE.md記録として残す（依頼書の
前提修正）。代わりに、実際に検知される既知の実データ例として
`tests/test_dcf_validity_checker.py`にALAB（CapEx 2023→2024、
$2.76M→$34.25M、倍率12.4倍、上記バックテストで実際に検知された6銘柄の
1つ）を用いた回帰テストを追加した。

#### テスト追加
- `tests/test_dcf_validity_checker.py`（新規）: `check_c_data_jump()`の
  後方互換性（Revenue呼び出しがパラメータ化前と完全に同一の挙動）・
  section/fieldパラメータ化・down_ratio非対称閾値（CapEx型）を検証する
  9件
- `tests/test_report_consistency_check.py`: 既存CHECK-21統合テスト3件の
  モックを`**kwargs`対応に更新（新規呼び出しの影響を受けないよう修正）、
  CHECK-44・CHECK-45それぞれの統合テストを新規5件追加

#### 検証結果（3点）
- `pytest -q`: 1163 passed（既存1149件＋新規14件）
- `audit.py`: NG無し（10銘柄警告、既存と同一）
- `report_consistency_check.py --fail-on-ng`: NG=0・WARN=119件
  （既存93件＋WARN-44/45新規26件、いずれも🆕未確認として個別レビュー
  対象。ベースラインWARN=93件の維持は本タスクの目的ではなく、新規
  検知の追加自体が目的のため件数増加は想定通り）
...
#### クローズ（2026-09-06）
16銘柄全ての一次情報裏取りが完了し、以下に帰着した：
- **JOBY・CEG・CPRT**（完全回収可能）: [[LAYER3-COGS-CANDIDATE-TAG-
  EXPANSION-1]]で実装済み・クローズ済み（BACKLOG_DONE.md参照）
- **VST**（部分回収可能）: ミスリードリスクのため意図的除外する設計
  判断とし、同じく[[LAYER3-COGS-CANDIDATE-TAG-EXPANSION-1]]で実装済み
- **CDNS・INTU**（標準タグだが恒久的にディメンション限定）: SECの
  companyfacts一括APIがディメンション付き事実を返さない構造的制約
  のため対応不可。本エントリでの文書化をもって対応終了とする
- **FLYW・XOM・VZ・BKNG**: カスタム拡張タグのため回収不可能と確定
- **V**: 原価概念自体が存在しないため、元のNEVER_REPORTED判定が正確
  であることを確認済み
- **CAKE・ASTS・LRCX**（既存3件）: 別エントリ（[[LAYER3-COGS-ASTS-
  LRCX-RECOVERABLE-FOLLOWUP-1]]等）で既に解決済み

[[JOBY-BLADE-ACQUISITION-IMPACT-SCOPE-1]]は本エントリとは別スコープ
（COGS以外の指標への波及確認）のため、独立エントリとしてBACKLOG.mdに
そのまま残す。
...
#### クローズ（2026-09-06）
Step 1〜6の対応がすべて完了している：Step 3・4でcore+satellite合計
53件のKPI取得成功まで改善。Step 5・6で残5件のうち3件（SOFI
Technology Platform・NCO・NIM）を解消、2件（ADBE研究開発費・APGE
営業費用）は`[[TAIL-THESIS-KPIS-EMPTY-ADBE-APGE-1]]`へ切り出し済み。
本エントリの核心だった「非ディメンション取得の構造的制約」自体は、
実際に調査した5件のうち原因だったものが0件と判明している（個別の
タグ精度問題だった）。残るoption(a)は「将来同種のケースが見つかった
場合に再検討」、SOFI NCO/NIMの抽出安定性は「次回以降の週次自動実行
での定期スポットチェック」——いずれも着手条件なしのwatch状態のため、
本エントリはこれ以上の能動的な作業を要さないと判断しクローズする。
...
- **データソース起因・真の取得失敗**: ~~ENB（IFRS/40-F企業のためSEC annual
  data 0件、`update.py`はエラーを出さず「完了」表示のまま空データを返す。
  P1-Step1-SEC NGで検出可能だが、Step 0（カナダ企業チェック）が本来
  弾くべきケース。列追加時点の遡及登録のため経緯不明のまま残存）~~
  2026-09-03 登録抹消（コミット`62aa662102`）により解消・[[CIK-ORPHAN-
  FLAGS-1]]参照
...
TICKER-AUDIT-1・PREFLIGHT-CHECK-1と統合的に設計すべき
（別々に実装しない）。
...
よる担保、技術的な強制はなし）。REGISTER-FLOW-REDESIGN-1の
対応方針としては全件着手完了となったが、本エントリ自体は
TICKER-AUDIT-1（2026-09-05に[[QUALITY-GATES-EPIC-1]]ゲート4の付録へ
統合済み、独立エントリとしては削除）・PREFLIGHT-CHECK-1との
統合的設計という当初の前提（上記「優先度についての所感」）の扱いが
残るため、クローズは見送り現状維持のままとする。

**（2026-09-05追記）** 上記「統合的設計」の相手だったTICKER-AUDIT-1・
PREFLIGHT-CHECK-1は、いずれも[[QUALITY-GATES-EPIC-1]]配下（ゲート4・
ゲート0）で実装が完了した（PREFLIGHT-CHECK-1は完了・BACKLOG_DONE.md
参照）。「統合的設計」という当初の前提自体が解消されたため、本エントリ
（REGISTER-FLOW-REDESIGN-1）のクローズ可否は次回セッションで
Koichiさんと再検討する余地がある（本タスクのスコープ外のため、
クローズの実施は見送る）。

#### 方針4実装（2026-09-03、cik_lookup.csvへのexclusion_reason列追加）

コミット`62aa662102`（ENB登録抹消本体）・`f0c4b18e09`（exclusion_reason
列追加＋stonks_silo方針文書化）。[[QUALITY-GATES-EPIC-1]]ゲート0対応の
一環として着手。

**ENB登録抹消が先行**: 方針4着手の過程で、上記「3. データソース起因と
プロセス起因の混同リスク」で「データソース起因・真の取得失敗」に分類
していたENBが、[[CIK-ORPHAN-FLAGS-1]]の自己点検（2026-07-10、yfinance
`country`フィールドでカナダ企業と確認済み）以降も未解決のまま残って
いることが再確認された。Koichiさんの判断により、BX（2026-07-11に
同一方針で解消済み）と同じくA案（登録抹消）を採用し、cik_lookup.csv
からENB自体を削除した上でexclusion_reason列を追加した（ENBは列追加の
対象外——存在しない銘柄に除外理由は不要なため）。詳細は[[CIK-ORPHAN-
FLAGS-1]]参照。

**exclusion_reason列の記入内容**: `config/cik_lookup.csv`
（103行、ENB抹消後）に新規列を追加し、4銘柄に個別の除外理由を記入した:
- RKLB・ZS: 成長株のためSTONKS SILO側で評価する設計
  （`tanuki=false`・`stonks_silo=true`。ZSは`eps=false`も同理由）
- SN: 20-F提出企業で四半期系列不足の一時的措置
  （[[SN-TANUKI-DELAY-1]]参照、Q2 10-Q提出後にtanuki=trueへ戻す予定）
- APGE: 売上ゼロの臨床段階バイオ、評価枠組み非適合（意図的除外）

それ以外の`tanuki=true`銘柄は空欄のまま。既存12列・103行の内容は
Pythonでのテキストベース部分追記（各行末に列を追加するのみ）で対応し、
csv.DictReaderでの往復検証・git diffで対象行以外の差分がないことを
確認した（`json.dump`全体書き直しがコンパクト表記を壊す事故〈2026-09-02
CIK履歴JSON編集時に発覚〉と同型のリスクを、CSVでも同じくテキストベース
部分編集を優先する方針で回避）。

**stonks_silo=falseは一般方針として文書化のみ、個別記入せず**:
`stonks_silo=false`は78銘柄（2026-09-03時点）と大多数を占めるが、これは
個別判断ではなく「大型株・黒字企業全般はSTONKS SILO対象外」という設計
方針そのものである（STONKS SILOは赤字企業の黒字化評価に特化したシステム
で、`stonks_silo=true`側〈25銘柄〉が例外的に選定された赤字企業リスト）。
cik_lookup.csvへの個別記入はせず、`SYSTEM_MAP.md`のSTONKS SILOセクション
に一般方針として1箇所明記した。

**検証**: pytest 1009 passed、registration_validator.py（103銘柄、
ENB関連NG/WARN皆無）・report_consistency_check.py --fail-on-ng
（NG=0、CHECK-32含め変更前と同一のNG/WARN件数）・audit.py・
...
#### クローズ（2026-09-06）
対応方針1〜5は上記の通り全て完了または実質達成しており（2026-09-03〜05
の一連の実装で確認済み）、クローズを保留していた唯一の理由——
TICKER-AUDIT-1・PREFLIGHT-CHECK-1との「統合的設計」という当初の前提
（上記「優先度についての所感」）——も、両者とも[[QUALITY-GATES-EPIC-1]]
配下（ゲート4・ゲート0）で実装完了したことにより解消済みとなった
（TICKER-AUDIT-1は2026-09-05にQUALITY-GATES-EPIC-1ゲート4付録へ全文
統合、PREFLIGHT-CHECK-1は同日ゲート0実装完了・BACKLOG_DONE.md「2026-
09-05⑤（完了）」参照）。クローズを妨げる要因が残っていないため、本
エントリをBACKLOG.mdからBACKLOG_DONE.mdへ移設する。
...
**状態:** ✅完了（[[QUALITY-GATES-EPIC-1]]ゲート0として実装）
**優先度:** 中
**分類:** 品質管理 / 銘柄登録フロー
**登録日:** 2026-07-02
**完了日:** 2026-09-05
...
不安定な傾向があることが判明した。ENB（IFRS/40-F企業、SEC annual data 0件の
まま遡及登録され孤立、2026-09-05に管理対象除外済み）は、本機能が実装されて
いれば登録前に検知できたはずの実例。
...
ENBは本タスクが実装されていれば①のフラグで登録前に弾けたはずの事例。一方
REGISTER-FLOW-REDESIGN-1・TICKER-SOURCE-UNIFY-1（完了・本ファイル参照）は
「プロセス起因（手順の飛ばし・銘柄リスト参照の取り違え）」を扱っており、
対象とする失敗モードが異なる。両者は補完関係にあり、どちらかで代替できる
ものではない。

#### 設計メモ追記（2026-07-15・ARCH-DATA-1残課題③調査結果を反映）
「ARCH-DATA-1のパターン判定ロジックを共有する」方針を見直した。SIC/
formerNamesベースの事前推測は精度未検証のリスクが高いと判明したため
見送り、revenue系タグ競合検知はStep1完了後の実データ検知
（`common/sec_data/revenue_tag_conflict_check.py`）に統合する方針とし、
本タスクの当初3項目（①上場後3年未満 ②直近フォームが20-F等
③revenueタグ不存在、いずれも登録時点の情報のみで判定可能）に限定した。
「未知パターン学習ループ」案（AIによる仮説生成→一次情報検証→カタログ
蓄積）は投機的なため、2026-09-05の実装時にスコープ外とした。

#### 実装（2026-09-05）
`common/registration/preflight_check.py`を新設し、①②③④を実装した。
`register_ticker.py::register_one()`のStep 0.5（cik_lookup.csv行の
ロード）直後・Step 1実行前に組み込んだ。

- **①**: SEC submissions APIの`filings.recent`から10-K/10-K/A/10-Q/
  10-Q/Aの最古filingDateを求め、そこから現在までの経過年数が3年未満
  ならフラグ。`filings.recent`（直近分のみ）に限定しても実用上十分
  （3年以内に上場した企業が直近1000件程度の提出履歴に収まらないほど
  申告することは現実的にない）
- **②**: 直近の「年次/四半期報告書」カテゴリ（10-K/10-K/A/10-Q/10-Q/A/
  20-F/20-F/A/40-F/40-F/A/6-K）の中で最新の提出が10-K/10-Q系でない
  場合にフラグ。8-K・SCHEDULE 13D・S-8等の無関係な提出書類まで
  「直近提出書類」として拾うと常時発火する無意味なチェックになるため
  （実装時にAPGEの直近提出が`SCHEDULE 13D/A`だったことで判明、これは
  同社の10-K/10-Q提出有無とは無関係）、このカテゴリに限定した
- **③**: `common/sec_data/parser.py::SECParser.XBRL_MAPPING["revenue"]`
  （cost_of_revenue等と同じ候補タグリスト）をそのまま流用し、独自の
  重複リストを作らずに収益系タグの存在有無を判定
- **④**: フラグが立ってもStep 1以降の処理は自動停止せず、警告表示のみで
  続行する。`run_preflight_check()`自体は例外を送出せず結果を返すのみ

**着手時に判明した実データとの乖離**: 依頼文の検証手順は「SN=20-F提出
企業」を②が検知する想定だったが、実際にSEC EDGARを確認したところ、
SNは登録日（2026-07-02）以降に10-K/10-Q提出企業へ移行済み
（初回10-K: 2026-03-02）であり、2026-09-05時点のライブ判定では②では
なく①（10-K/10-Q提出履歴が浅い）が発火する。20-F提出企業だったという
過去の事実に由来する「四半期系列不足」という同一の根本リスクを、②では
なく①が捉える形になっている。②のロジック自体は現役の20-F/6-K提出企業
（BABA、テスト用に手動確認）で実際に発火することを別途確認済みであり、
デッドコードではない。

#### 検証結果
- 実データ（SEC EDGAR）で確認: APGE→③発火（Revenues等の候補タグが
  1件も存在せず）、CON→①発火（初回10-K/10-Q提出日2024-08-27、経過
  約2.0年）、SN→①発火（初回10-K提出日2026-03-02、経過約0.5年。②では
  なく①が発火した経緯は上記「実データとの乖離」参照）、AAPL（正常な
  既存銘柄）→フラグなしを確認
- `tests/test_preflight_check.py`（20ケース）: ①②③それぞれの検知・
  非検知条件、8-K/13D等の無関係書式を②が誤検知しないこと、CIK未登録・
  API到達失敗時にエラーとフラグを区別して記録すること、副作用
  （ファイル書き込み等）がないことを合成データで検証
- `tests/test_register_ticker_preflight.py`（4ケース）: register_one()
  がStep 0.5直後・Step 1実行前にプリフライトを呼ぶこと、フラグが
  立っても登録処理を自動停止しないこと、cik_lookup.csv行が存在しない
  場合の既存の早期return挙動が壊れていないこと、Step1失敗時の既存の
  中断挙動が壊れていないことを確認
- `python -m pytest -q`: 1142件全パス（新規24件含む）
...
#### 実装（2026-09-05）
実装せず、対応不要としてクローズ。機能への影響がない純粋な見た目調整の
ため、Koichiさんの判断により対応不要とした。
...
- **LITE（net_income）**: end=2015-06-27（本人データ、fy=2015）と
  end=2015-08-01（fy=2018の10-Kで参考開示、is_own_data=False。登録時
  「10-Q」と誤記していたが2026-09-05の一次情報確認で10-Kと判明・是正
  済み）が同一年度バケツで競合
...
#### 一次情報確認結果（2026-09-05）
LITE・WSTそれぞれの該当filingをSEC EDGAR原本で直接確認した。両銘柄とも
**判定1（単発の参考開示・タグ付けミス、対応不要）に該当し、未確認の
決算期変更は実在しないと確定した。**

**LITE（accn 0001633978-18-000108）**: EDGARで実際の form type を確認
したところ、内容欄の記載「fy=2018の10-Qで参考開示」は誤りで、**実際は
FY2018 10-K**（期末2018-06-30、提出2018-08-28）だった（本文の記載も
是正）。同10-KのFilingSummary.xml→R98（Quarterly Financial Information
footnote）を確認したところ、end=2015-08-01は**2015年8月1日のJDS
Uniphase（JDSU）からのスピンオフによるLumentum Holdings独立時点**を
指すことが判明。company_facts.jsonのNetIncomeLoss全履歴を確認すると、
`start=2015-06-28, end=2015-08-01`（$-11.7M、スピンオフ直前の
5週間弱の端数期間）と`start=2015-08-02, end=2016-07-02`（$21.0M、
スピンオフ翌日から本決算期末までの11ヶ月）が別エントリとして存在し、
合算するとFY2016通期（`start=2015-06-28, end=2016-07-02`、$9.3M）と
完全に一致する（$-11.7M+$21.0M=$9.3M）。これはスピンオフに伴う
Predecessor/Successor会計処理（GAAP上の標準的な分割開示）そのもの
であり、以後のFY2017・FY2018 10-Kで比較年度データとして繰り返し
再掲されているだけと確認した。LITEの決算期アンカー（6月末〜7月頭の
52/53週決算）自体はスピンオフ前後で一切変化しておらず、決算期変更は
実在しない。

なお、company_facts.jsonの精査中に`config/warn_acknowledged.json`に
既存のLITE向けWARN-21登録（2026-07-12付、コメント「2021年会計年度末
変更に伴う移行期間の正規の10-K提出」）を発見した。これは本エントリ
（WARN-24、2015年スピンオフ由来）とは**別のチェック・別の事象**
（2021年の実際の決算期変更、98日間の移行期スタブ、[[SEC-TAG-FICO-
CPRT-1]]完了記録参照）であり、依頼書が言及していた「2026-08-30の
別セッションでLITE分は部分的に解消済みと指摘された」という食い違いは、
このWARN-21登録（LITEの別の会計年度末変更事象）とWARN-24
（本エントリ、2015年スピンオフ事象）の混同に起因すると推測される。
本エントリの対象（2015年end=2015-08-01の競合）はこれまで一度も
対応・登録されておらず、「部分的に解消済み」という記載の裏付けは
BACKLOG_DONE.md・git commit履歴（2026-08-25〜09-01）のいずれにも
見つからなかった。

**WST（accn 0000105770-23-000012）**: EDGARで確認したところFY2022
10-K（期末2022-12-31、提出2023-02-21）で正しい（内容欄の記載と一致）。
「rpo」フィールドは`DeferredRevenue`/`ContractWithCustomerLiability`
系タグにマッピングされており、company_facts.jsonを確認すると
end=2013-06-30・val=$20,000,000が**2013年当時のWST自身の10-Q
（accn 0000105770-13-000041、`DeferredRevenueCurrent`$1.5M+
`DeferredRevenueNoncurrent`$18.2M）に由来する本人データ**であり、
2014年〜2017年のfilingでは`DeferredRevenue`（合算タグ）として比較
年度に繰り返し再掲されていた（正常な経年比較開示）。ところが**2022年
Q3 10-Q（accn 0000105770-22-000054）以降、ASC 606移行後の
`ContractWithCustomerLiability`タグの下に、同じend=2013-06-30・
val=$20,000,000が再度出現**し、FY2022 10-K・FY2023 Q3・FY2023 10-Kまで
同一の値が繰り返されている。しかしFY2022 10-KのFilingSummary.xml→R52
（「Revenue - Contracts Assets and Liabilities (Details)」）を実際に
確認したところ、**表示されている期間はDec.31,2021とDec.31,2022の2列
のみで、2013年のデータは一切表示されていない**。つまりこの
end=2013-06-30の`ContractWithCustomerLiability`エントリは、開示書類
の人間が読める本文のどこにも現れない、**XBRL側にのみ存在する非表示の
タグ付けアーティファクト**と確認した。2013年当時の`DeferredRevenue`
タグ用に作成された古いXBRLコンテキスト（期間定義）が、ASC 606移行後の
`ContractWithCustomerLiability`タグへ誤って使い回された（filing
agentのXBRL生成テンプレートの不具合と推測される）ことが原因と考えられ、
WSTの決算期（12月末）自体は一貫しており決算期変更は実在しない。

**結論**: LITE・WSTともに判定1（対応不要）で確定。`fye_change_
candidate_scan.py`のクラスタリング閾値見直しやticker_restrictions対応
は不要と判断する。

#### クローズ（2026-09-05追記）
一次情報確認により両銘柄とも判定1が確定したため、`config/warn_
acknowledged.json`へWARN-24のLITE・WST向けエントリを新規登録した上で
本エントリをクローズする。
...
#### クローズ（2026-09-03追記）
[[QUALITY-GATES-EPIC-1]]ゲート0対応（[[REGISTER-FLOW-REDESIGN-1]]方針4）
の過程で、[[CIK-ORPHAN-FLAGS-1]]（ENBの全システムフラグfalse孤立登録）
の解消としてKoichiさんの判断によりENB自体を登録抹消した（コミット
`62aa662102`）。正規化データが1件も生成されない原因（本エントリの
本題）自体は未調査のまま残るが、ENBが登録から完全に消滅したため
原因調査の対象が消滅し、これ以上の対応は不要と判断してクローズする。

---

### ✅ [CIK-ORPHAN-FLAGS-1] BX・ENBが全システムフラグfalseの孤立エントリ
**優先度:** 低〜中（BX・ENBとも解消済み）
**分類:** データ品質 / 銘柄登録
**登録日:** 2026-07-10
**完了日:** 2026-09-03
...
2026-07-11時点ではENBのみが未解消だった（2026-09-03に解消、下記
「ENBの解消（2026-09-03追記）」参照）。
...
コミット`62aa662102`（登録抹消本体）・`f0c4b18e09`（[[REGISTER-FLOW-
REDESIGN-1]]方針4実装、記録は同エントリ参照）。[[QUALITY-GATES-EPIC-1]]
ゲート0対応の過程でENBの孤立登録が再確認され、Koichiさんの判断により
BXと同じくA案（登録抹消）を採用した。上記「対応方針」節で検討していた
B案（TANUKI-FIN-1でのDDM型評価への振り分け）は、TANUKI-FIN-1自体が
本タスク時点で未着手のまま持ち越されており、ENBを評価不能な孤立状態で
これ以上待たせる合理性がないと判断されたため見送りとなった。

削除後にregistration_validator.pyを再実行し、ENB関連のNG/WARNが0件に
なったことを確認済み（対象銘柄数104→103）。P4-CIKOrphanチェックも
ENBを正しく検出しなくなったことを確認した。

**本エントリの対象（BX・ENB）はいずれも解消済みのためクローズする。**

---

## 2026-09-02④（完了）

### ✅ [TRUST-SUMMARY-EPIC-1] データ信頼性の3段階（入力完全性・成長率算出・FCF/DCF計算）を貫通する可視化EPIC — FCF-CONVRATE①②③全実装完了・判断保留4件解消確認によりクローズ
**優先度:** 高（要設計、実装未着手）
...
**クローズ（2026-09-02）**: ①②③すべて実装・検証・push完了により、本EPICを
クローズする。

**①②③の対応コミット**:
- ①（rate_is_sector_defaultフラグ追加）: 2026-09-02実装完了
  （コミット`547736e376`）
- ②（report.txt/stock.html表示不一致修正）: 2026-09-02修正完了
  （コミット`39e9eee08f`）
- ③（fcf_conversion_config.jsonへDamodaran比較メタ情報追加）:
  2026-09-02実装完了（コミット`7167df09ae`）

**判断保留4件の解消確認**: GROWTH-SANITY-CLASS-SYNC-1（2026-07-19完了）・
FY52WEEK-BS-NULL-SILENT-1（Phase A・B Stage1-3・C全完了、2026-07-20までに）・
MRVL-2019-2020-NULL-1（同）・EPS-ANALYZER-NORMALIZE-SCOPE-1（同）は
いずれも本EPICと無関係な別セッションでBACKLOG_DONE.mdへ完全移動済み
（✅マーク付き見出しで確認済み）。

**骨子①（可視化前のバグ切り分け）の最終到達点**: 2026-07-18棚卸しで
15項目を再検証し、8件を解消可能なバグとして個別タスクへ分離
（FYE-CHANGE-BOUNDARY-COLLISION-BLIND-1・CASH-TAG-MISSING-1・
REVENUE-TAG-PRIORITY-FRAGILE-1・WORKFLOW-SEC-TANUKI-GAP-1・
FCF-CONVRATE①・MA-INTEGRATION-TAG-GAP-1・SPLIT-REALTIME-GAP-1・
FCF-OUTLIER-PREROUNDING-LOSS-1、いずれも完了）、構造的限界として残った
1件（FCF-CONVRATE②）のみを可視化対象に絞り込んだ。

**骨子②（構造的限界の可視化）の最終到達点**: 段階1（成長率算出）は
GROWTH-STRUCTURAL-MISMATCH-CANDIDATES-1（2026-07-20完了、
`GROWTH_STRUCTURAL_MISMATCH_TICKERS`方式）、段階2（FCF/DCF計算）は
FCF-CONVRATE②（`FCF_CYCLICAL_VOLATILITY_TICKERS`方式、SITM/LITE限定、
2026-07-18実装）＋①（`rate_is_sector_default`フラグ方式、47銘柄検知）＋
③（Damodaran比較の開発者向けメタ情報、画面表示は変更なし）で対応し、
個別ティッカーリスト方式とフラグ方式の両パターンが確立された。段階0
（データ完全性）はFY52WEEK-BS-NULL-SILENT-1等の個別バグ修正で対応
（可視化ではなく根本解消のため骨子①側の扱い）。

以上により、EPIC登録時（2026-07-12）に定義した段階0〜2・骨子①②の
スコープ内で残る未対応項目はなく、クローズする。
...
#### 続き（2026-09-03、実地確認結果・判断保留）
上記「申し送り（次回サイクル待ち）」①（次回平日実行の完了時刻が
JST7:00より前か）について、直近2日分の実行結果を確認した。

- 2026-09-02: JST08:31:05完了
- 2026-09-03: JST08:30:38完了

想定していたJST6:35頃（UTC21:35+9h）に対し、2日連続で約1時間55分の
遅延が発生しており、当初の目標（朝7時台に最新データを見る）は現時点
では未達。

**判断は保留とする**。理由: 2日分のみでは、この遅延パターンが恒常的
（cron設計の見直しが必要）か一時的（GitHub Actions基盤側の一過性の
混雑）かを判断する材料として不十分なため。次回確認は1週間程度
（複数の平日サイクル）経過後を目安とする。以下の選択肢はいずれも
保留のまま記載しておく:
- cronのさらなる前倒し
- 7時台目標自体の見直し
- 追加観察の継続（現状維持）

申し送り②（次の土曜朝にJST金曜分データが反映されているか）は本追記
時点で未確認のため、上記と合わせて次回確認時にまとめて実施する。
...
正式登録、2026-09-01）決定により、Discoverサブシステム自体が削除対象と
なったため対応不要と判定しクローズした。実装は行っていない（記録の移設
のみ）。各エントリの元の内容は削除せず保持し、末尾に判定文を追記した。
...
[[DISCOVER-SUBSYSTEM-REMOVAL-1]]決定（2026-09-01）によりDiscover
サブシステム自体が削除対象となったため、本タスクは対応不要と判定し
クローズする。
...
**優先度:** 低 → 中 → 完了（Koichiさん判断で優先度引き上げの上、
即実装完了。2026-08-30、SEC_Data_Update障害調査で3週間分のデータ
滞留が誰にも気づかれなかった実害が確認されたことを受けた判断）
**分類:** 運用・監視 / GitHub Actions
**登録日:** 2026-08-29
**完了日:** 2026-08-30

#### 内容（登録時点）
GitHub Actions基盤側の一時的障害（2026-08-26〜28の複数インシデント）
によりワークフローが遅延した際、「Koichiさんが実際に画面を見て初めて
気づく」以外の検知手段がないことが判明。登録時点ではアイデア段階
（優先度低）だった。

#### 優先度引き上げの経緯（2026-08-30）
`SEC_Data_Update`週次ワークフローが2026-08-16・08-23の2回連続で
Consistency Check Gateステップで失敗し、`Commit and push changes`
ステップに到達できず、約3週間分（2026-08-09以降）のSEC EDGARデータ
更新が本番に反映されないまま誰にも気づかれず放置されていたことが
判明した（詳細は本ファイル内`[[SEC_Data_Update障害調査]]`関連の
2026-08-30コミット群を参照、原因はワークフローの依存パッケージ
インストール不足）。この実害を受け、Koichiさんが優先度引き上げ・
即対応を判断した。

#### 実装内容
`common/system_health.py`（毎日実行される既存のSystem Health Check
ワークフロー、`.github/workflows/System_Health.yml`）に新規チェック
`check_j_workflow_runs()`を追加した。

- `.github/workflows/*.yml`から`cron:`定義済みワークフローを機械的に
  列挙（新しいcronワークフローが今後追加されても自動的に監視対象に
  含まれる）
- 各ワークフローの直近の完了済み実行をGitHub Actions REST API
  （`GET /repos/{owner}/{repo}/actions/workflows/{file}/runs`）で取得し、
  (1) 直近実行が失敗（`conclusion != success`）していないか、
  (2) cron式の頻度（毎日/週数回/週次/月次を曜日・日付フィールドから
  簡易推定）に応じた許容日数を超えて未実行になっていないか、を判定
- GITHUB_TOKEN環境変数があれば認証付きで（レート制限緩和）、なければ
  匿名で呼び出す（匿名でも公開リポジトリのメタデータ取得は可能なことを
  実測確認済み）
- 検知時の通知は新規の外部サービス連携を追加せず、既存の仕組みのみで
  完結: (a) System_Health.pyが元々持つDiscord Webhook通知（既存機能を
  再利用、新規連携ではない）、(b) 異常時はスクリプトの終了コードが
  非ゼロになりGitHub Actions上でワークフロー実行自体がRED表示される
  （既存のSystem_Health.yml自体の設計を踏襲）
- `System_Health.yml`自体は監視対象から除外（実装後の実地確認で、
  同ワークフローはWARNING相当でもexit 1になり毎日のようにrun自体が
  failure表示になる設計だと判明し、これを含めるとcheck Jが常時RED化
  し信号として機能しなくなるため）。自身が完全に停止した場合の検知は
  この仕組みの原理的な限界として残る（対応方針検討時から認識していた
  トレードオフ）

#### 実地確認（2026-08-30、本番のGitHub Actions REST APIに対して実行）
ローカルから実際に`python common/system_health.py`を実行し、本番の
GitHub Actions REST APIに対してcheck Jを動作させたところ、
`SEC_Data_Update.yml`の直近完了実行が`failure`（2026-08-23の障害、
本チケットの発見契機そのもの）として正しく検知された。これは
モックではなく実データによる検知確認であり、本チェックが実際に
機能することを裏付けている。

#### 回帰テスト
`tests/test_system_health_workflow_monitor.py`（新規、10ケース）。
GitHub Actions REST API呼び出し部分（`_fetch_latest_run`）をモックし、
(1) 失敗実行の検知、(2) 許容日数超過（未実行）の検知、(3) 正常系での
異常なし判定、(4) API到達不可時に異常扱いしない設計の確認、(5) cron式
からの頻度推定ロジック（毎日/週数回/週次/月次）の単体検証、を行う。
修正前（本チェック追加前のHEAD、コミット`c95017d823`時点）の
`common/system_health.py`にはこれらの関数が一切存在しないことを
`git show`で確認済み（＝修正前は必ずAttributeErrorで失敗し、修正後は
全10件成功する回帰テストとして機能する）。

#### 既存チェック（A〜I）との関係
既存のcheck D（ワークフロー"出力データ"のgit commit日付ベースの鮮度
チェック）は対象が主要4系統の出力ファイルに限定されており、
SEC_Data_Updateの出力は対象外だった。今回追加したcheck Jは
「ワークフロー自体の実行結果」をGitHub Actions REST API経由で直接
確認するため、check Dより対象範囲が広く（cron定義済み全ワークフロー）、
判定もより直接的（実行成否そのもの）である。既存チェックとは独立して
併存させ、置き換えは行っていない。

#### 未対応として残す事項（スコープ外）
実地確認の過程で、`[F] TailCtrl`（APGE分のctrl/latest.json不足）・
`[G] HypeCore`（17日更新なし、閾値14日）という既存の別警告が現在も
継続していることを確認した。これらは本タスクのスコープ外（既存の
別問題）であり、対応していない。Koichiさんへの報告時に情報提供のみ
行う。
...
#### 着手条件
なし（parser.py側は解消済み。本番データ再生成も上記「Step: 本番データ
再生成（2026-08-22、コミット`610c18c71`）」で同日中に実施済み——旧版の
本欄は「別セッションで実施」としていたが、実際には同一セッション内で
完了したため2026-08-26のブラッシュアップで訂正した。Layer3側は
`[[LAYER3-OI-RECONSTRUCTION-FALLBACK-GAP-1]]`へ切り出し済み・そちらの
着手条件を参照）。
...
#### 発見経緯・関連
`[[QUALITY-GATES-EPIC-1]]`Phase4追加調査（2026-08-20⑤）でCHECK-40
...
**状態:** 完了（2026-08-20②。`route_rejected_to_layer3()`に`xbrl_
dimension`非空ガードを追加し再発防止。既存誤同定2件〈APP・CELH〉を
是正し、実害があった両銘柄のレビューを再生成。下記「対応（2026-08-20②）」
参照）
**優先度:** 中→解消
...
#### 追加調査（2026-08-20③）: dimensionガードの穴の確認
...
2026-08-20⑥、`[[QUALITY-GATES-EPIC-1]]`ゲート3棚卸しの追加調査
...
**状態:** 完了（方針確定: 全保有ポジションを監視対象とする。core/
satelliteはthesis内のポジション重み付けとして残すが監視対象の決定には
使わない）
**優先度:** 中（データ破損ではなく運用上の欠落だったが、保有ポジション
への実カバレッジ欠落を解消）
**分類:** 運用ギャップ / TAIL自動化パイプライン / 方針判断待ち→完了
**登録日:** 2026-08-19
**完了日:** 2026-08-19②
**発見:** `[[TAIL-SATELLITE-POSITION-MONITORING-GAP-1]]`（本ファイル
上部）で一度「対応方針(b)＝core限定維持」が採用されたが、その判断が
ユーザーに提示した対象範囲の誤認（1銘柄と説明→実際は7銘柄）に基づいて
いたため差し戻された（`CHAT_RULES.md`事例6参照）。本項目はその差し
戻しを受けて実測データを添えて再登録し、改めてユーザーの方針判断を
仰いだ結果、全保有ポジションを対象とする方針が確定した。

#### 経緯（差し戻し前の誤認）
前項目でCowork側はユーザーに「satellite種別（現状APGE）」＝1銘柄の
話として方針判断を仰いだが、`get_core_tickers()`の実測値は
`['PLTR', 'SOFI', 'TSLA']`の3件のみであり、RSS監視・四半期レビュー
自動生成の対象から除外されていたのは実際には**CELH・APP・NVDA・ADBE・
SOUN・CRWV・APGEの7銘柄＝保有10銘柄中7銘柄**だった。

#### 採用された方針（2026-08-19②、ユーザー判断）
**全保有ポジション（10銘柄）をRSS監視・四半期レビュー自動生成の対象と
する。** core/satelliteの区別はポジションの重み付けとしてthesisに残す
が、監視対象の決定には使わない（保有している以上、決算は見る）。

#### 実測結果（2026-08-19②、方針決定時点、保有10銘柄すべて）

| ticker | 種別 | レビュー件数 | 最終レビュー四半期 | 未レビュー決算数<br>（2023-01-01以降） | 最終SEC提出日 | 監視されていない期間 |
|---|---|---|---|---|---|---|
| ADBE | satellite | 0 | なし | 15 | 2026-06-15 | 2026-06-06〜（74日） |
| CELH | satellite | 0 | なし | 15 | 2026-05-07 | 2026-06-06〜（74日） |
| APP | satellite | 0 | なし | 14 | 2026-05-06 | 2026-06-06〜（74日） |
| NVDA | satellite | 0 | なし | 14 | 2026-05-20 | 2026-06-06〜（74日） |
| SOUN | satellite | 0 | なし | 14 | 2026-05-11 | 2026-06-06〜（74日） |
| APGE | satellite | 0 | なし | 12 | 2026-05-11 | 2026-07-02〜（48日） |
| CRWV | satellite | 0 | なし | 5 | 2026-05-08 | 2026-06-06〜（74日） |
| PLTR | core | 9 | 2026Q2 | 0 | 2026-08-04※ | なし（監視中） |
| SOFI | core | 9 | 2026Q2 | 0 | 2026-08-06※ | なし（監視中） |
| TSLA | core | 9 | 2026Q2 | 0 | 2026-07-23※ | なし（監視中） |

（2026-08-19時点、経過日数の起算日は「今日」＝2026-08-19。
「未レビュー決算数」は最古の実績レビュー対象四半期〈2023Q3〉に合わせ
2023-01-01以降の10-Q/10-K提出件数〈`company_facts.json`の`form`が
10-Q/10-Q-A/10-K/10-K-Aのエントリを`filed`日ベースで重複排除した件数〉。
※core3銘柄の「最終SEC提出日」は`review_queue.json`が記録する実際の
処理履歴〈`filed`フィールド〉。ローカルの`company_facts.json`キャッシュ
は3銘柄とも2026年5月時点までしか反映されておらず、CIが実行時に取得
する最新データより古い既知の乖離だが、本項目の比較そのものには影響
しない）

#### Step 1-3/1-4（差し戻し前の実測、方針確定の根拠として保持）
`git log -p`で`review_queue.json`の全履歴・`docs/portfolio/tail/data/
reviews/`のファイル追加履歴を検索した結果、satellite 7銘柄は最初から
一度も自動レビューされたことがない（全履歴を通じて0件）。
`edgar_rss_monitor.py`は2026-06-06新設、CELH/APP/NVDA/ADBE/SOUN/CRWVは
2026-06-05登録・APGEは2026-07-02登録で、いずれも`type: "satellite"`は
登録時点から一貫して変更なし。7銘柄とも監視の仕組みが存在するように
なって以降、一度も監視対象になったことがなかった。

#### 実施した変更（2026-08-19②）
**Step 0（呼び出し元洗い出し）**: `get_core_tickers()`の呼び出し元は
2箇所のみ（`edgar_rss_monitor.py::main()`＝監視対象の決定、
`report_consistency_check.py`のCHECK-37＝監視対象と同じ入口を使う
検査）。それ以外に見つかった`type=="core"`比較は監視対象の決定とは
無関係と判定し変更対象から除外: `kpi_proposer.py::propose_kpis()`
（別機能のKPI提案生成、本依頼のスコープ外として未着手）、
`workflow_write.py::register_position()`（ポジション登録時の必須
フィールド分岐、データ入力スキーマであり監視対象決定ではない）。

1. **`edgar_rss_monitor.py`**: `get_monitored_tickers()`を新設
   （`positions_index.json`の全ポジションをtype無関係に返す）。
   `get_core_tickers()`は削除せず存置（他用途のため）。`main()`の
   `--ticker`未指定時の分岐を`get_monitored_tickers()`に差し替え、
   `監視対象: {n}銘柄（全保有ポジション）`の1行サマリーを追加。
   `get_excluded_positions()`は固定の`type!="core"`比較から
   `get_monitored_tickers()`の実際の戻り値との差分に再設計——現時点
   では常に空リストを返すが、将来`get_monitored_tickers()`側に何らかの
   除外条件が追加されても、本関数を個別修正せず自動的に検出できる
   （沈黙除外の再発防止をget_monitored_tickers()の変更に追随する形で
   維持）。実測で`--ticker`未指定時の対象が3銘柄→10銘柄になったこと、
   `get_core_tickers()`自体は変更されず3銘柄のままであることを確認済み。

2. **satelliteでのレビュー生成〜DCFシナリオ生成の実地検証（本依頼で
   最も重要な工程）**: `latest.json`存在確認を satellite 7銘柄全てで
   実測（NVDA/ADBE/APP/CELH/SOUN/CRWVは存在、APGEのみtanuki=falseで
   不在）。ADBE（`latest.json`あり）・APGE（`latest.json`なし、
   RSS実測で判明した実際の最新提出＝2026Q2、accn=`0001193125-26-
   341289`、filed=2026-08-10、ローカルの`company_facts.json`キャッシュ
   は2026-05-11までしか反映されておらずAPGEも既知のキャッシュ遅延が
   あった）で、実際に`edgar_rss_monitor.py --ticker`実行→
   （差分検知設計により初回実行はキュー追加なしのため）実データの
   accnを`review_queue.json`へ手動投入→`quarterly_review_generator.
   py`実行、という経路を最後まで実際に流し、両銘柄ともレビュー
   ファイルが生成されることを確認した。ADBEはその後のDCFシナリオ
   生成にも成功（`docs/portfolio/scenario/ADBE/{base,bear,bull}.json`、
   内容確認済み・欠損フィールドなし）、APGEは`latest.json`不在により
   `[SKIP]`で正常にスキップ（クラッシュなし）。

3. **実地検証中に発見・修正した実バグ**: 最初の生成では、ADBE・APGE
   両方のレビューでGrokが「投資テーゼが完全に未設定」と評価し
   health_score大幅減点・recommendation=EXITという内容になった。原因を
   追跡した結果、`quarterly_review_generator.py`の`build_stage1_
   prompt()`・`build_call2_prompt()`が`thesis.get('thesis', ...)`・
   `thesis.get('entry_story', ...)`・`thesis.get('exit_guide', ...)`と
   coreスキーマのフィールド名を直接読んでおり、satelliteスキーマ
   （`strategy_name`/`entry_condition`/`exit_condition`/
   `holding_period`）のthesisファイルに対しては全て存在しないキーで
   デフォルト値「未設定」が返っていたことが判明した。`_thesis_
   narrative_fields()`を新設してtype別にフィールドを正規化するよう
   修正し、再生成した結果、両銘柄ともテーゼ内容を正しく反映した評価
   （ADBE: health_score 15→45・EXIT→WATCH、APGE: health_score
   10→25・EXIT〈理由が「テーゼ未設定」から実質的な懸念点に変化〉）に
   なることを確認した。これは「入口を広げても下流で止まる」典型例を
   実地検証で発見・修正できたケースであり、修正しなければ全10銘柄への
   拡大は見かけ上動くが内容が破綻した状態になっていた。

4. **CHECK-37（`report_consistency_check.py`）の対象範囲更新**:
   `get_core_tickers()`から`get_monitored_tickers()`に変更。docstring
   内の「2026-08-19決定 core限定」等の旧方針記述を更新。**過渡期の
   扱い**: 監視対象拡大前から蓄積していた旧satellite 6銘柄分の未レビュー
   決算在庫（上表参照）は、`edgar_rss_monitor.py`の差分検知設計（初回
   実行はベースライン記録のみでキュー追加なし）により自動的には解消
   されないため、これをNGとして出し続けるのは不適切と判断し、
   `latest_filed`が方針変更日（2026-08-19）以降の提出のみをNG判定対象
   とする条件を追加した。方針変更日より前の既存在庫は一律NG化しない。
   実装後、10銘柄全てで実行しNG=0を確認済み。

5. **`.github/workflows/TANUKI_TAIL_RSS_Monitor.yml`の確認**: RSS監視
   ステップは`--ticker`未指定で動くため、Step 1の変更により自動的に
   10銘柄が対象になることを確認（ワークフロー自体の変更不要）。
   `workflow_dispatch.inputs.ticker`のdescription文言のみ「全Core銘柄」
   →「全保有ポジション」に更新。tail_dcf_bridgeステップ
   （`os.listdir('docs/value-monitor/tanuki_valuation/data')`ベース）
   は実測の結果、保有10銘柄中9銘柄（APGE以外全て）が既にこの一覧に
   含まれていることを確認——`generate_scenario_files()`自体が
   `latest.json`必須である以上、APGEを一覧に含めても結果は変わらない
   ため、**前回の「変更不要」判断は今回も有効と確認し変更しなかった**。

#### 未対応のまま残した点（今回のスコープ外）
- **旧satellite 6銘柄（NVDA/APP/CELH/SOUN/CRWV、ADBEは今回実測検証で
  対応済み）の既存未レビュー決算在庫**は、次回以降の新規提出から
  順次自動的に監視が始まる設計であり、今回は一括バックフィルを行って
  いない（ユーザー指示は satellite1銘柄での実地検証〈ADBE・APGE〉の
  みを求めており、全銘柄の全既存在庫の一括処理は本依頼の明示スコープ
  外と判断）
- `kpi_proposer.py::propose_kpis()`のcore限定ゲートは、本依頼が対象と
  した「RSS監視・四半期レビュー自動生成」とは別のKPI提案機能であり、
  今回は変更していない。同様の対象拡大が必要かはユーザー判断待ち

#### 関連
- `[[TAIL-SATELLITE-POSITION-MONITORING-GAP-1]]`（本ファイル上部、
  本項目の元となった項目）
...
**状態:** 完了（最終確定）。実装部分（除外の沈黙を解消する`[SKIP]`
ログ明示化・監視漏れ検出`CHECK-37`）は2026-08-19時点で完了。**方針
判断は一度「(b)core限定維持」を採用したが、対象範囲の誤認（1銘柄と
説明→実際は7銘柄）に基づいていたため差し戻し、`[[TAIL-COVERAGE-
POLICY-UNDECIDED-1]]`として再登録・改めて方針判断を仰いだ結果、
2026-08-19②に「全保有ポジション（10銘柄）を監視対象とする」方針が
確定し、実装・実地検証まで完了した（詳細は`[[TAIL-COVERAGE-POLICY-
UNDECIDED-1]]`本ファイル参照）。**
**優先度:** 中（データ破損ではなく運用上の欠落だったが、保有ポジション
への実カバレッジ欠落は完全解消済み）
**分類:** 運用ギャップ / TAIL自動化パイプライン / バグ
**登録日:** 2026-08-19
**完了日:** 2026-08-19②（実装部分は2026-08-19に先行完了、方針判断は
一度差し戻された後、`[[TAIL-COVERAGE-POLICY-UNDECIDED-1]]`で
2026-08-19②に最終確定・実装完了）
...
#### 対応方針(b)の決定（2026-08-19、ユーザーの投資方針判断）→ 差し戻し（2026-08-19②）
core/satelliteの区別はTAILの投資方針そのもの（core=主力ポジション、
satellite=補助的ポジション）であり、satellite種別のレビュー自動生成を
行わないことは**意図的な設計**であって欠陥ではない、との判断で方針(b)
（レビュー生成をcore限定のまま維持）が一度採用された。

**この判断は誤った前提の上で行われたため差し戻された（2026-08-19②）。**
ユーザーに判断を仰ぐ際、Cowork側は対象を「satellite種別（現状APGE）」＝
1銘柄として説明したが、`get_core_tickers()`の実測値は3銘柄
（PLTR/SOFI/TSLA）のみであり、除外されているのは実際には**CELH/APP/
NVDA/ADBE/SOUN/CRWV/APGEの7銘柄＝保有10銘柄中7銘柄**だった。ユーザーは
「1銘柄の話」という誤った規模認識のまま方針(b)を選んでおり、判断の
前提自体が誤っていた。詳細は`[[TAIL-COVERAGE-POLICY-UNDECIDED-1]]`
（本ファイル上部、2026-08-19②完了）および`CHAT_RULES.md`事例6を参照。

**「APGEのレビューが0件である状態は今後も継続する」という記述は誤りと
して削除する。** 方針は一度未決定に戻った後、`[[TAIL-COVERAGE-POLICY-
UNDECIDED-1]]`で「全保有ポジションを監視対象とする」方針が2026-08-19②
に確定し、実地検証としてAPGEのレビューを実際に生成した
（`APGE_2026Q2_review.json`、実データとしてコミット済み）。**APGEの
レビューが0件である状態は解消済み。** 将来この節を見た人が誤った過去の
記述に基づいて再び混乱しないよう、経緯として保持する。

#### 実施した対応（2026-08-19、方針差し戻し後も有効）
方針の当否とは独立に、**除外が沈黙している**（何も出力せずスキップする）
状態は本セッションで潰してきたサイレント・フォールバックと同型の問題
として修正した。**この実装部分は方針(b)の差し戻しとは無関係であり、
そのまま有効**——除外対象がどの銘柄であっても、除外自体をログで明示
する・監視漏れを検出するという2つの仕組みは方針の内容に依存しない。
**判定ロジックは一切変更していない**——除外される銘柄の
...
- `[[LAYER3-ANNUAL-CLASSIFICATION-DROPS-DATA-1]]`（同じ調査の一部として
  ...
  本項目はその流れで見つかった別種の問題）
- `SYSTEM_MAP.md`「TANUKI TAIL」節に監視方針を記録（2026-08-19②時点で
  確定版に更新済み。全保有ポジション対象、方針差し戻しの経緯を含む）
- `[[TAIL-COVERAGE-POLICY-UNDECIDED-1]]`（本ファイル上部、2026-08-19②
  完了。本項目の方針判断を差し戻して再登録し、全保有ポジション対象と
  いう方針が確定・実装完了した項目。satellite 7銘柄の監視実態の実測
  データ、実地検証結果を含む）
...
**状態:** 調査完了・対応不要（現状維持）
**優先度（訂正）:** 高 → **低**（2026-08-16、調査結果に基づき訂正）
...
登録時点では「TANUKI VALUATIONの理論株価計算（AS-IS-002/004/006等、
**全銘柄のDCF計算の中核**）に組み込まれる定数」として優先度「高」で
登録されていた。

#### 優先度引き下げの根拠（2026-08-16調査、読み取り専用）
登録時の背景記述「全100銘柄のDCF計算の中核」は**不正確**だった。
実コード追跡の結果:

- **メインIV**（`intrinsic_value_per_share`＝AS-IS-001、
  `upside_percent`＝AS-IS-006「統一クラスタの唯一の正」）は
  `market_return`（10%固定）のみで割り引かれており、
  `risk_free_rate`はこの計算経路に**一切登場しない**
  （`core_calculator.py:495, 640-641`で実測確認）
- `risk_free_rate`が効くのは参考表示3項目のみ:
  `intrinsic_value_beta`（AS-IS-002、参考①）・
  `intrinsic_value_rf`（AS-IS-004、参考②）・`erp`
  （`pipeline.py:967`のコード内コメントで「フロント参考表示用」と
  明記）
- `_compute_tanuki_score()`（TANUKI SCORE分類）は
  `valuation.get("upside_percent")`のみを参照し、上記3項目は判定に
  使われていない

#### 実装の実態
- `0.043`は4箇所に散在（`wacc.py:57`のデフォルト引数・同ファイルの
  `DEFAULT_RISK_FREE_RATE`定数・`pipeline.py:972`と`:2246`の
  フォールバック）だが、**値は全て一致**しており不整合はない
- `config/`から読む経路は存在せず、完全にコード内固定
- `wacc.py`の全履歴3コミットはいずれもGitHub Web UI一括アップロード
  （「Add files via upload」）で、初回コミット`4116fdaf2`
  （2026-04-12）時点で既に`0.043`。値の変更履歴はなく、設定根拠を
  示す記録はコミットメッセージからは追えない（未確認）

#### 感度の実測（AAPL・KO、2026-08-16、生成データは非コミット）

| ティッカー | β | Rf 0.043→0.045のWACC変化 | 参考①IV変化 |
|---|---|---|---|
| AAPL | 1.086 | −0.017pp（逆方向） | +0.26% |
| KO | 0.342 | +0.132pp（順方向） | −3.42% |

**設計上の性質（バグではない、必読）**: `WACC = Rf + β×(Rm-Rf)`という
実装のため`dWACC/dRf = 1-β`となり、**β>1の銘柄ではRfを上げると
β込みWACC（参考①）が下がる**という直感に反する符号反転が起きる。
これは`Rm=10%固定`（市場リスクを意図的に除外した本源価値、
`CLAUDE_CODE_START.md:831`）という設計の必然的帰結（Rmを固定した
ままRfを動かすとERP=Rm-Rfが縮むため）であり、将来この符号反転を
見て「計算式がおかしい」と誤って修正しないこと。

#### 対応方針（確定: A. 現状維持）
以下の理由により、**正規化固定レートとして意図的に維持する**（コード
変更なし）:
- 影響が参考表示3項目に限定され、主IV・TANUKI SCORE分類は無傷
- `Rm=10%固定・市場リスク除外`という既存の一貫した設計方針と整合
- B（config化）・C（macro_data連携）はいずれも実装コストに見合う
  実益がない。特にCは`common/macro_data/`に`DGS10`が**未収集**
  （`DGS1`・`T10Y2Y`等はあるが`DGS10`はない、2026-08-16確認）のため
  新規系列追加が前提条件になり、参考表示のみへの影響のためにこの
  コストを払う価値は薄い

#### 記録の反映
`FIELD_DEFINITIONS.md`のrisk_free_rate関連注記段落に、上記内容
（メインIVへの非影響・意図的な設計判断である旨・β>1銘柄での符号
反転）を追記済み。
...
### ✅ [PHASE2-MIGRATION-POLICY-DECIDED-1] フェーズ2（過去データ移管）の移行方針を確定・記録
**状態:** 完了（方針決定のみ。個別データソースの実装は別エントリで管理）
**優先度:** 高（新DB構築プロジェクト フェーズ2全体の実装方針を左右する）
**分類:** アーキテクチャ / 新DB構築プロジェクト フェーズ2
**登録日:** 2026-08-12
**完了日:** 2026-08-12
**発見:** フェーズ2着手前投資調査（文書ベース→実データ検証、チャット
記録、2026-08-12）

#### 経緯
フェーズ1（`common/sec_data/`・`common/market_data/`・
`common/macro_data/`）完成後、フェーズ2「過去データ移管」に着手する前に
以下の順で投資調査を実施した:

1. **文書ベース調査**: `PROJECT_STATUS.md`・`INPUT_DATA_TOBE.md`・
   `INPUT_DATA_AS_IS.md`・`BACKLOG.md`を確認したが、「移管」の定義
   （物理コピーか、再取得による切り捨てか）・完了判定基準のいずれも
   文書上に記載がないことが判明
2. **実データ検証（仮説: 移管が必要なのはSEC EDGARのみ）**:
   - yfinance旧保存先を5ファイルサンプル確認した結果、5ファイル中3
     ファイル（hypecore・`market_data.json`・`breadth_data.json`）が
     既に数ヶ月〜32ヶ月分の時系列データを保持していることが判明
   - FRED旧`05_events.csv`（34,297行）を確認した結果、ほとんどの指標が
     1996年、Sahm Ruleは1949年まで遡る深い履歴を保持していることが判明
   - SEC EDGARの`company_facts.json`（生アーカイブ）とannual/
     quarterly_*.jsonをAAPL/MSFT/XOM3銘柄で比較した結果、有意な深度差は
     確認できず
   - 上記仮説（SEC EDGARのみ移管が必要）は**反証**された
3. **common/macro_data/の履歴深度の実測**: 全24系列（FTSD除く）中、
   `BAMLH0A0HYM2`のみが785件（`2023-08-14`〜）と著しく浅いことを発見
4. **原因調査**: `fredapi::get_series_info()`のnotes欄に「Starting in
   April 2026, this series will only include 3 years of observations.」
   と明記されていることを確認。FRED側（データ提供元ICE Data Indices）が
   2026年4月から提供範囲を直近3年に制限したことが根本原因と判明
   （取得側・検証ロジック側の不具合ではないことをコード読解・
   `macro_data_violations_log.json`実測の両面で確認済み）
5. **全系列への横展開確認**: 全25系列（FTSD含む）についてFRED公式
   `observation_start`・notes欄を機械的に調査した結果、同種の新規制限に
   該当するのは`BAMLH0A0HYM2`のみ（`SP500`は別種の既知・恒久的な10年
   制限に該当するが対象外）と確認

#### 決定内容
上記の調査結果を踏まえ、フェーズ2の移行方針を以下の通り確定した:
...
- **例外**: データ提供元が恒久的にAPI経由の提供範囲を制限しており、
  ...
  一度限りの例外的移行を行う。移行したレコードにはprovenanceの
  `source_detail`に例外的移行である旨を明記し、ライブ取得分と区別できる
  ようにする
- **前提**: 旧保存先データはフェーズ2完了後に不要となったものから順次
  削除する方針のため、削除前にこの判断を完了させる必要がある

各データソースの現状評価（詳細は`PROJECT_STATUS.md`フェーズ2表参照）:
- **SEC EDGAR**: 作業不要の可能性が高いが未確定（3銘柄サンプルのみ、
  全105銘柄の精査は未実施）
- **yfinance**: 再取得による対応を検討中（次段階の調査対象）
- **FRED**: `BAMLH0A0HYM2`のみ例外規定を適用し`05_events.csv`から
  一度限りの移行を実施予定（`[[MACRODATA-BAMLH0A0HYM2-HISTORY-
  EXCEPTION-1]]`として別途登録）。他23系列は再取得のみで対応可能
- **取得前提条件**: 対象外（ファイルサイズが小さく、移管・再取得いずれも
  不要）

#### 対応内容
`PROJECT_STATUS.md`フェーズ2セクションへ移行方針の説明を追加し、表の
備考欄へ各データソースの現状評価を追記。`[[MACRODATA-BAMLH0A0HYM2-
HISTORY-EXCEPTION-1]]`をBACKLOG.mdへ新規登録（未着手、実装は次段階）。

実装コード変更・データ変更なし（ドキュメント記録・方針決定のみ）。
...
### ✅ [BACKLOG-STALE-NEXTSTEPS-BLOCK-MACRODATA-2] BACKLOG.md「次セッションでの着手順序」ブロックの陳腐化が再発（PROJECT_STATUS.md内部矛盾も同時発見）
**状態:** 完了
**優先度:** 低（ドキュメント記載の陳腐化・内部矛盾、実データ・実装への
実害なし。ただし次回セッションが誤参照するリスクあり）
**分類:** ドキュメント不備
**登録日:** 2026-08-12
**完了日:** 2026-08-12
**発見:** `[[MACRODATA-BAMLH0A0HYM2-HISTORY-EXCEPTION-1]]`完了後の
セッション終了時ブラッシュアップ調査（チャット記録、2026-08-12）

#### 内容
セッション終了時ブラッシュアップで、以下2件の不整合を発見した。

1. **PROJECT_STATUS.md内部の矛盾**: フェーズ2表のFRED行（`BAMLH0A0HYM2`
   の例外的移行について「実施済み」と記載、コミット`2ff481b3a`で更新）
   の直後にある説明文が、その前のコミット`5fff1bd53`（方針決定のみ）
   時点のまま「実装（`BAMLH0A0HYM2`の例外的移行含む）は次段階に分離、
   本更新は方針決定・記録のみ」と記載されており、同一ファイル内で
   矛盾していた
2. **BACKLOG.mdの「次セッションでの着手順序」ブロックの陳腐化**:
   `[[MACRODATA-LAYER-CONSTRUCTION-1]]`エントリ内のブロックが、
   本番消費者切替完了コミット`19e1dd8f5`時点の内容のまま、以降3コミット
   （`7751e2603`〈陳腐化ブロック修正〉・`5fff1bd53`〈フェーズ2方針決定〉・
   `2ff481b3a`〈`BAMLH0A0HYM2`実装〉）を反映せず残置されていた。
   「1. 次は新DB構築プロジェクトの残フェーズ検討」という、既に完了した
   内容を指す記述のままだった

2件目は、前回発見・修正した`[[BACKLOG-STALE-NEXTSTEPS-BLOCK-
MACRODATA-1]]`（`[[MARKETDATA-LAYER-CONSTRUCTION-1]]`エントリ側の陳腐化
ブロック）と**同種の問題の再発**である。

#### 対応内容
- PROJECT_STATUS.mdの矛盾していた説明文を、`BAMLH0A0HYM2`実装完了の
  事実・残タスク（下記2件）への参照に更新
- BACKLOG.mdの「次セッションでの着手順序」ブロックを、フェーズ2方針
  確定・`BAMLH0A0HYM2`完了・フェーズ2残タスク2件を反映する内容に更新
- フェーズ2の残タスク2件を新規登録: `[[PHASE2-SECDATA-FULL-DEPTH-
  VERIFICATION-1]]`（SEC EDGAR全105銘柄の履歴深度精査）・
  `[[PHASE2-YFINANCE-REFETCH-DESIGN-1]]`（yfinance旧保存先の時系列
  データ再取得設計）

#### 教訓（根本原因の仮説・再発防止の提案、判断・ルール化はチャット側で行う）
**根本原因の仮説**: macro_data関連タスクが完了するたびに、当該タスクの
完了報告（PROJECT_STATUS.md・BACKLOG.mdの該当エントリ更新）は都度
行われているが、それとは別に存在する「次セッションでの着手順序」という
**ナビゲーション用の独立したブロック**の更新は、完了報告のチェック
リストに明示的に含まれていないため、都度忘れられやすい。前回
（`[[BACKLOG-STALE-NEXTSTEPS-BLOCK-MACRODATA-1]]`）・今回
（`[[BACKLOG-STALE-NEXTSTEPS-BLOCK-MACRODATA-2]]`）とも、同一の
`[[MACRODATA-LAYER-CONSTRUCTION-1]]`関連の作業が数コミットにわたって
連続する中で発生しており、単発のタスクでは発生しにくく、**同一
プロジェクトに対する複数回・連続する完了報告**の際に特に発生しやすい
パターンと見られる。

**再発防止の提案（例）**:
- 完了報告の定型チェックリストに「関連する`[[...]]`エントリの
  『次セッションでの着手順序』ブロックが最新化されているか」の項目を
  追加する
- `grep -n "次セッションでの着手順序" BACKLOG.md`を完了報告の都度
  機械的に実行し、直近で更新されていないブロック（同一プロジェクトに
  関連するもの）がないかを確認する運用を、セッション終了時に限らず
  各完了報告のタイミングでも行う
...
**分類:** 移行に伴う意図せぬ改善（登録当初は「データ設計上の問題」として
分類していたが訂正）
**登録日:** 2026-08-11
**訂正・完了日:** 2026-08-11
**発見:** `[[MARKETDATA-LAYER-CONSTRUCTION-1]]`着手順序4-8
（hypecore.py本体切替）のStep3検証、104銘柄全数比較中に発見・登録。
直後の投資調査で前提の誤りが判明し訂正・クローズ（いずれもチャット
記録、2026-08-11）

#### 内容（訂正後の正しい理解）
当初、`common/market_data/fetcher.py`のdaily/層がauto_adjust=False
（未調整終値）で保存されている一方、切替前のhypecore.py・
breadth_calculator.py本体はauto_adjust=True（調整済み終値）を使用して
いたことによる乖離を「market_data層側の設計不備」として登録した
（TASK: 特別配当$3.65で乖離率24.8pt差、AAPL: 0.2pt差、他複数銘柄で
確認）。

しかし、後続の事実確認調査で**前提が誤っていた**と判明した:

- 200日移動平均等のテクニカル指標が知りたいのは「実際に取引された
  価格」の推移である。配当落ち日の株価下落は、生の終値
  （unadjusted close）に既に正しく反映されている（その日実際に
  その価格で取引された、という事実そのもの）
- 調整済み終値（adjusted close）は「配当を再投資し続けたと仮定した
  場合のトータルリターンを1本の価格系列で表す」ための、異なる目的の
  指標であり、過去に遡るほど生の終値より人為的に低く計算される
  （実測確認: `Adj Close`/`Close`比率が配当権利落ち日ごとに階段状に
  変化し、直近日は常に1.0〈無調整〉であることをAAPLの実データで確認。
  yfinance側`auto_adjust`のデフォルトは`True`であることもソース
  コードで確認済み）
- したがって、**hypecore.py・breadth_calculator.pyが旧実装で調整済み
  終値を使っていたことの方が、純粋なテクニカル指標（移動平均乖離率）
  の計算としては元々不適切だった**。今回のmarket_data層
  （auto_adjust=False、生の終値）への移行は、この設計上の欠陥を
  意図せず是正した形になる

#### 結論
- **対応不要**。現状（auto_adjust=False）が正しい設計であり、変更しない
- 案A（auto_adjust=Trueへの変更）は検討不要（むしろ改悪になる）
- TASK 24.8pt・AAPL 0.2pt等の観測差分は、旧実装の欠陥が是正された
  結果の差分として記録する（バグの証拠ではなく、改善の証拠）
- 影響範囲調査で洗い出した他の切替済み消費者（data_fetcher.py・
  collect_and_send.py・breadth_calculator.py）についても、同様に
  「旧実装の不適切な調整済み終値使用が是正された」という前向きな
  結果として整理し、個別の追加対応は不要と判断する

#### 検証
訂正の根拠となった事実確認調査で以下を実測確認済み:
- `Adj Close`/`Close`比率が配当権利落ち日ごとに階段状に変化し、直近日は
  常に1.0（無調整）であることをAAPLの実データで確認
- `breadth_calculator.py`母集団から独立抽出した40銘柄で「NEW（未調整）
  <OLD（調整済み）」の方向性を検証、逆転0件
...
#### 内容
`[[JNJ-RD-TAG-PRIORITY-1]]`（本ファイル参照）で修正した
...
独立した候補タグリスト）にもそのまま残っていた。

#### 影響
JNJのように両タグを並存報告する銘柄で、損益計算書本体の主要R&D科目
（ExcludingAcquiredInProcessCost）ではなく、IPR&D即時費用化等の一時的
項目（ResearchAndDevelopmentExpense単体）が優先採用され、
research_and_developmentが実態の約1/30に過小計上される
（`[[JNJ-RD-TAG-PRIORITY-1]]`と同型の実害）。フェーズD（TANUKI
VALUATION本体のLayer3切替）着手に伴い顕在化することを見越して先行修正。

#### 対応（実装完了、2026-08-06）
`config/sec_concept_definitions.json`の`fields.research_and_development.
candidates`の順序を、`tag_definitions.py`と同様に
`ResearchAndDevelopmentExpenseExcludingAcquiredInProcessCost`優先へ
入れ替えた。105銘柄全数の前後差分比較で、影響がJNJの1銘柄のみである
ことを確認済み（AMD/AVGOにも同型のタグ競合があったが、いずれも6年
retention窓の外の古いデータのみで実害なし）。pytest 505 passed/2
known failed（既知）。
...
period_days=180〜273）が混入していた。fp='FY'タグ付きの比較開示
...
gross_margin_3yr_avgが変わっていた。

#### 対応（実装完了、2026-08-06）
`layer3_builder.py`側の後処理として`_reclassify_misannotated_fy_
entries()`を新設し対応（BBAI限定、共有関数`_classify_period()`/
`_process_entries()`自体は変更せずnormalized/への影響を回避）。判定
基準は「同一accn・同一start日内に複数end日付を持つfp=='FY'・130〜
300日エントリ」（`accn`のみでのグルーピングは、SPAC合併等の
predecessor/successor2エンティティの正当なstub決算〈start日が
異なる〉を誤って巻き込むことをBBAI自身の2020年データで確認したため、
`start`日を要件に追加）。105銘柄×32フィールド全数の前後差分比較で、
影響がBBAI（revenue/gross_profit以外にoperating_cash_flow・
net_income等14フィールドに同型の問題を確認・修正）とJNJ（RD、
`[[LAYER3-CONFIG-RD-TAG-PRIORITY-1]]`、別件）以外に及ばないことを
確認済み。pytest 505 passed/2 known failed（既知）。

BBAI以外に同型の判定漏れが確認された銘柄は`[[LAYER3-ANNUAL-
MISCLASSIFICATION-NOW-RMBS-1]]`（NOW・RMBS）・`[[LAYER3-ANNUAL-
MISCLASSIFICATION-MINOR-5TICKERS-1]]`（ASTS/SPIR/DELL/VRT/META）
としてBACKLOG.mdに別途記録・保留。
...
除外するフィルタを実装した。影響範囲をpipeline.py内の希薄化率計算に
限定し、Layer3スキーマ自体（他消費者・他用途が参照する可能性がある
`shares_diluted`フィールド全体）には触れていない。

不採用の案: Layer3の候補タグから`CommonStockSharesOutstanding`を除外する
案（スキーマ側の根本修正）は、`shares_diluted`を参照する他用途への影響
範囲の洗い出しが別途必要になるため見送り。将来的にLayer3の
...
### ✅ [SEC-EDGAR-LAYER-DESIGN-PHASE-D-STEP2-1] フェーズD Step2-1: TANUKI VALUATION本体切替（reader.py・pipeline.py）
**状態:** pipeline.py 6箇所の切替完了。次はフェーズD Step2-2
（STONKS SILO）
**優先度:** 高（本線）
**分類:** アーキテクチャ / 新DB構築プロジェクト フェーズ1
**完了日:** 2026-08-06
**発見:** `SEC_EDGAR_LAYER_DESIGN.md`フェーズD Step2-1実装依頼（チャット記録）

#### 事前バグ修正
`[[LAYER3-CONFIG-RD-TAG-PRIORITY-1]]`（JNJ RDタグ優先順位）・
`[[LAYER3-ANNUAL-MISCLASSIFICATION-BBAI-1]]`（BBAI is_annual誤分類）を
本切替の前提条件として先行修正（本ファイル該当エントリ参照）。

#### 実装内容
`pipeline.py`の6箇所を`layer3_builder.py::get_field_entries()`経由に
切替（normalized/への直接アクセスを全廃）:
1. 希薄化率計算（SharesDiluted）: quarterly側は`[[LAYER3-
   SHARESDILUTED-TAG-GAP-1]]`案2どおりsource_tagフィルタを適用
2. TTM信頼性判定（NetIncome）: フィルタは既存動作維持（is_annualのみ
   除外、is_ytd除外は本切替のスコープ外・別途判断）
3・4. LTDebtフォールバック（`_calc_g_fundamental()`・
   `_calc_roic_wacc_ratio()`、共通メソッド）: `_get_normalized_lt_debt()`
   を`_get_lt_debt_fallback()`に改名し、新設`get_long_term_debt_
   latest()`（Layer3優先、normalized/へのフォールバックなし。
   RCAT/SPIR/CPRTのSEC EDGAR照合結果に基づく設計）に切替
5. `_estimate_ttm_operating_income()`（GrossProfit・RD・SM）
6. `_calc_moat_inputs()`（GrossProfit・Revenue）

`__init__`にticker単位のLayer3ストアキャッシュ
（`_layer3_store_cache`）を追加し、同一tickerでの
`company_facts.json`重複パースを回避。

#### 全数回帰結果（100銘柄、実際に発火するゲート条件を踏まえた検証）
- `_calc_g_fundamental`（G）: 0/100差分
- `dilution_3yr_annual_pct`・`split_factor`・`is_split`: 0/100差分
  （分割検知トリガー発火9銘柄の実アルゴリズムシミュレーションで確認、
  `[[LAYER3-SHARESDILUTED-TAG-GAP-1]]`参照）
- TTM信頼性判定（dupont.reliability）: 0/100差分（NetIncome四半期差分
  5銘柄はいずれも`quarters_used==4`のゲート条件自体を満たさず無関係）
- `moat_gross_margin_3yr`: 7/100差分、全て`[[LAYER3-GROSSPROFIT-
  BACKFILL-MISSING-1]]`等の既知理由でLayer3側がより正確（改善）
- `roic_wacc_ratio`・`moat_roic`: 4/100差分（COHR/LLY/JNJ/KLAC、値→
  Noneに変化）。原因はLayer3の`selling_and_marketing`が正しく空判定
  される（`_estimate_ttm_operating_income()`のGrossProfit-RD-SM
  intersectionが0件）ことによる既知のSM/SGA概念混同問題の帰結。
  ユーザー判断により選択肢1（現状維持、Noneのまま受け入れる）を採用。
  詳細は`[[LAYER3-ROIC-WACC-NONE-4TICKERS-1]]`参照

検証当初、フォールバック関数を無条件に呼び出す方法で59銘柄もの
差分を検出したが、これは`bs.get("long_term_debt") or fallback(...)`
の短絡評価を無視した誤った検証方法だったため、実際に呼ばれる
高レベルメソッドを直接比較する方式に訂正して再検証した。

#### テスト結果
pytest 505 passed/2 known failed（既知の`[[TEST-STALE-IV-1]]`のみ）。
実装直後に新規失敗1件（`TestDuPontReliabilityLowFlag`、テスト側が
normalized/フィクスチャに依存していたため）を発見し、
`build_ticker_store()`をpatchする方式に修正して解消。
`report_consistency_check.py --fail-on-ng`: NG=0・WARN=78件（既存と
不変）。

#### 残タスク
フェーズD Step2-2（STONKS SILO）以降はBACKLOG.mdに
`[[SECDATA-STORAGE-FRAGMENTATION-1]]`として同一IDで残置。
...
**状態:** 実装完了（`_bs_identity_extra_components()`にcross-accnフォール
バック＋2段階ガードを実装。機能コミット`b63be0026`・データコミット
`05e7f853d`。COHR(2022/2023)・CRWV(2024)・VRT(2018)の4件を解消）
**優先度:** 中
**分類:** アーキテクチャ改善 / CHECK29設計方針の拡張
**登録日:** 2026-08-02
**完了日:** 2026-08-03
**発見:** [[CHECK29-UNRESOLVED-23-MIXED-CAUSES-1]]個別調査（チャット記録）

#### 内容
CHECK29の「本人データ（own accn）限定」照合という設計方針により、一時的
持分・NCIタグが当該年度自身の10-Kではなく後続四半期の比較列としてのみ
開示される場合（COHR型、M&A・組織再編直後の合併対価としての優先株式等）
を原理的に検知できなかった。COHR(2022/2023)で実例確認済み: 自社own accn
の10-Kには該当タグが存在しないが、後続四半期filing（2022年分は次四半期
10-Q、2023年分はFY2024 Q2 10-Q）の比較列としては一貫した値が報告されて
おり、その値を用いるとTA=TL+SE+extraが完全一致する（2022年:
$766,803,000、2023年: $2,241,415,000）。

#### 設計調査（2026-08-03、チャット記録・読み取りのみ）
own-accn限定を外して同一end_dateの全filingを横断検索する緩和設計を、
既存の`bs_identity_violations_log.json`（本番生成済み・105銘柄・156
エントリ）に対してオフラインシミュレーションした。結果、COHR以外に
**CRWV(2024)・VRT(2018)の計2銘柄3件が同型パターンで新たに該当**すると
判明（いずれも`TemporaryEquityCarryingAmountAttributableToParent`タグが
後続filingの比較列としてのみ開示されるM&A/スピンオフ直後のケース）。

安全性検証: CHECK29は検知専用ロジック（`_check_bs_identity_violations()`
のdocstring「検知専用（自動修正なし）」の通り、annual_YYYY.json等の実
データには一切書き戻さず、`report_consistency_check.py`の非ブロッキング
WARN-29としてのみ消費される）であるため、誤った値を拾っても実害は
「本来出すべきWARNを誤って消す」に留まると確認した上で、実装を推奨した。

#### 実装1回目で発見した回帰（2026-08-03、チャット記録・重要な教訓）
依頼された設計（own-accn一致0件の場合のみ全accn検索に広げるフォール
バック）をそのまま実装し、全105銘柄で再生成・本番ログとの厳密比較
（dict key順序を無視した意味的diff）を行ったところ、**意図した4件以外に
9件の変化があり、うち5件（SOUN2021・PM2010/2011・TSLA2020/2021・
HEI2014・FCX2015）は既存の正しい解消結果（`resolved_by_extension=true`・
diff=0）を壊す回帰**だった。

根本原因は2種類:
1. **別タグ族での同額重複計上**（FCX2015が典型）: own-accnで既に
   `MinorityInterest`+`TemporaryEquityCarryingAmountIncludingPortion...`
   で完全一致していたのに、フォールバックが後続filingから
   `RedeemableNoncontrollingInterestEquityCarryingAmount`という別タグ族で
   同額$764,000,000を新たに拾い、二重計上した。既存の
   `_BS_IDENTITY_SUPERSEDES`は特定2組のタグペアしかカバーしておらず、
   この種の「別タグ族での同額重複」全般には対応していなかった。
2. **own-accnのみで既に完成していた解への不要な追加**（SOUN2021・
   PM2010/2011・TSLA2020/2021・HEI2014が該当）: own-accnのみの値で恒等式
   が既に厳密一致（diff=0）していたにもかかわらず、フォールバックが
   （既存値と重複しない）新規の値を無条件に追加してしまい、正しい解を
   壊した。単純な「値の重複チェック」だけでは防げないパターンで、
   17件のみ（当時の未解消ケース）を対象としたchat記録シミュレーションで
   は検出できず、**全母集団（既に解消済みの約88件）への影響確認を怠って
   いた**ことが原因と判明。

この時点でコード変更・再生成データを`git checkout`で実装前の状態に復元
し、コミットせずに一時停止・ユーザーへ報告した。

#### 2段階ガード再設計と再シミュレーション（2026-08-03、チャット記録）
以下の2段階ガードを設計し、既存の`bs_identity_violations_log.json`
156エントリ全件へのオフラインシミュレーションで、回帰5件の再発防止と
意図した4件の解消の両立を確認してから実装した:

1. **ベースゲート（新設）**: own-accnのみで収集した値の合計で恒等式が
   厳密に一致（diff=0）する場合、そのentity（年度）に対してはcross-accn
   フォールバック処理自体を一切実行しない（年度単位で丸ごとスキップ）。
   回帰5件はすべてこのゲートで防止される（own-accnのみで既にdiff=0
   だったことを確認済み）。
2. **重複値ガード**: ①を通過した場合でも、フォールバックが見つけた候補
   値が既にmatched済みの他タグの値と一致（許容誤差0.1%または$1,000
   以内）する場合は採用しない（FCX型の別タグ族重複への追加の安全策）。

#### 実装・検証結果（2026-08-03）
`_bs_identity_extra_components()`にベースゲート＋重複値ガードを実装し、
全105銘柄をローカルキャッシュの`company_facts.json`のみで再パース
（SEC API再取得なし）。全105銘柄・全156エントリの意味的before/after
比較（dict key順序を無視）で、変化は以下の6件のみと確認:
- COHR(2022): `resolved_by_extension` False→True、diff $766,803,000→$0
- COHR(2023): 同上、diff $2,241,415,000→$0
- CRWV(2024): 同上、diff $1,722,000,000→-$111,000
- VRT(2018): 同上、diff $666,911,522→$0
- TSLA(2016): 既存True維持、diff $8,784,000→$0（精度改善、副次効果）
- ASTS(2020): 既存False維持、diff $150,596,928→$148,106,928
  （複数filing一致でも恒等式が解消しない偽陽性が正しく弾かれることを
  再確認）

前回の回帰5件（SOUN2021・PM2010/2011・TSLA2020/2021・HEI2014・FCX2015）
は今回の156件中に一切現れず（本番値と完全一致）、ベースゲートにより
再発防止を確認。上記6件以外の150件は完全に変化なし。annual_YYYY.json等
の実データファイルは1件も変化なし（検知専用ロジックの想定通り）。

pytest 497 passed/2 known failed（MSFT/NVDA、既知・無関係）。
`report_consistency_check.py`実行: WARN-29 17件/11銘柄→13件/9銘柄に減少
（COHR・CRWVの2銘柄が完全解消、VRTは2件→1件に減少）。

#### コミット
- 機能変更: `b63be0026`
- データ再生成（10銘柄のbs_identity_violations_log.json。うちFCX/HEI/
  ONDS/PM/RDWは値・resolved判定に変化なし、[[BS-IDENTITY-LOG-
  NONDETERMINISTIC-KEY-ORDER-1]]既知のfrozenset反復順序非決定性による
  キー順序差異のみ、または一部タグがより精度の高い候補への置換のみ）:
  `05e7f853d`

#### 教訓
候補修正のシミュレーション範囲を「対象の未解決ケースのみ」に限定すると、
既存の正しい解を壊す回帰を見逃す。フィルタ条件・探索範囲を拡張する変更
は、対象外の既存解決済みケース全体（全母集団）への影響を必ず確認して
からコミットすること（CHAT_RULES.md「候補修正の全母集団シミュレーション」
の実例として追加）。設計段階のchat記録シミュレーションが「対象17件のみ」
で完了と判断していたことが、実装1回目の回帰を招いた直接原因。
...
**状態:** 実装完了（`_last4_is_contiguous()`新設・`calc_ttm_series()`へ
挿入。機能コミット`e2892a91f`・データコミット`426b4fa2f`。RCAT・KULRの
IV影響はΔIV=$0と実測確定、FROGは投資適格性の結論に変化なしと確認）
**優先度:** 中〜高
**分類:** バグ / 確定・一般的な設計欠陥
**登録日:** 2026-08-02
**完了日:** 2026-08-02
...
#### 実装完了報告（2026-08-02）
`ttm_calculator.py`に`_last4_is_contiguous()`（合計スパン305-425日・
隣接ギャップ±10日）を新設し、`calc_ttm_series()`の`last4`選定直後に
挿入（機能コミット`e2892a91f`）。`eps_basic`/`eps_diluted`は
`CONTIGUITY_CHECK_EXEMPT_FIELDS`として明示的にチェック対象外とした
（実装時に発見: 当初この除外を実装し忘れ、全105銘柄再生成テストで
99/105銘柄が誤って影響を受けることが判明し即座に是正。既知・実害
なしの仕様との整合を最終確認）。代替候補探索ロジックは実装しない
（設計確定どおり）。

該当18銘柄のTTM系列を再生成（データコミット`426b4fa2f`）。RCAT・KULR・
FROGの該当anchorでFCFが正しく不完全（quarters_used不足）として除外
されることを確認。現行パイプラインを保ったまま新旧`ttm_calculator.py`
ロジックを個別実行して分離比較する方式で、対象18銘柄以外の87銘柄・
PEP等の正当ケースが完全に無変化であることを確認済み（コミットには
対象18銘柄分のみを含め、それ以外は意図的に据え置いた。ttm/ディレクトリ
全体が2026-07-26生成のまま既存の別コミット〈2026-07-30〜〉に対して
陳腐化していたことが判明したが、これは本タスクのスコープ外のため
対応せず据え置く）。

**RCAT・KULR・FROGのIV影響を実測**: `pipeline.py`を試験実行（コミット
なし）した結果、RCAT・KULRいずれも`FCF補正`ロジック（`FCF外れ値:
latest_negative`検知時の代替推定オーバーライド）が発火し、修正前後の
fcf_5yr_avgの値（RCAT: -$40,185,008→-$16,477,214、KULR: -$26,088,907→
-$20,850,327）に関わらず、補正後の値・理論株価（RCAT: $3.73、KULR:
$4.53）・乖離率が**完全に同一**であることを確認した。**ΔIV=$0を実測で
確定**（RCATは既報告どおり・KULRも同様の帰結と判明、当初のBACKLOG登録
時点で想定していた「約20%改善がIVに反映される」という見立ては誤りで
あり本実装で訂正する）。FROGは元々TTM系列の完全点数が3点で
`min_years=3`を満たしたまま（年次実績へのフォールバックは発生しない、
当初の想定とは異なる帰結）、fcf_avgが$120.3M程度の黒字を維持し投資
適格性の結論に変化なし。

`report_consistency_check.py` NG=0・WARN 81件（変化なし）、pytest 519
passed/2 known failed（MSFT/NVDA、既知・本修正と無関係）を確認。

---

### ✅ [KULR-CAPEX-TTM-STUB-ENTRY-CONTAMINATION-1] KULRのcapital_expenditure四半期系列に開始日欠落の異常エントリが混入し正規四半期を押し出している
**状態:** 解消（[[TTM-CALC-QUARTER-CONTIGUITY-UNCHECKED-1]]の実装で
自動解消。ΔIV=$0を実測確認済み）
**優先度:** 高
**分類:** バグ / 確定・現在進行形の実害
**登録日:** 2026-08-02
...
#### 実装完了報告・IV影響の最終確定（2026-08-02）
[[TTM-CALC-QUARTER-CONTIGUITY-UNCHECKED-1]]の実装（機能コミット
`e2892a91f`・データコミット`426b4fa2f`）により、個別対応なしで自動解消
された。KULRのcapital_expenditureの該当anchor（2023-03-31・2024-03-31）
は正しく`quarters_used`不足として除外されることを確認。

**IV影響を実測し訂正**: 当初「fcf_avgが約20%改善する」と見立てていたが、
`pipeline.py`実行による実測の結果、`FCF外れ値: latest_negative`検知時の
代替推定オーバーライドロジックが、fcf_5yr_avgの値（-$26,088,907→
-$20,850,327）に関わらず同一の補正後値（$1,293,632）・同一の理論株価
（$4.53）・同一の乖離率（+71.0%）を出力することを確認した。**ΔIV=$0を
実測で確定**（Classificationも当然変化なし）。データ品質バグ自体は
確定・修正済みだが、KULRの現行IV・投資判断への影響は結果的にゼロだった
と結論する。

---

### ✅ [COHR-SHARES-DILUTED-UNIT-SCALE-BUG-1] COHR自身のFY2011 10-Kがshares_diluted等を実際の1/1000でタグ付けしていた本人データ側の単位スケール申告誤り
**状態:** 実装完了（`fact_overrides.json`個別上書き、COHR(2009-2011)の
shares_diluted/shares_basicを是正。機能コミット`82e25d92d`・データ
コミット`3896f7393`）
**優先度:** 中
...
#### 実装前最終確認結果（2026-08-02、チャット記録、読み取り・オフライン
シミュレーションのみ）
[[XBRL-UNIT-SCALE-MISMATCH-DETECTION-1]]側の実装前最終確認で、既存の
恒等式ベース安全網との相互作用リスクなし・ガード適用後シミュレーションで
該当はCOHRの2010年度2フィールドのみと確定。tie-break変更は2010年度1件
しか解決せずfact_overrides側で重複解決されるため、tie-break変更（
ソースコード変更）は当面見送り、`fact_overrides.json`個別上書きのみを
実装対象とすることを確定した。

#### 実装完了報告（2026-08-02）
`fact_overrides.json`にCOHR(2009-2011)のshares_diluted/shares_basicを
追加（機能コミット`82e25d92d`）。GOOGL-FACT-OVERRIDE-SEQUENCING-BUG-1で
確立済みの`_apply_fact_overrides()`（抽出直後・全逆算バックフィルより
前で実行）がそのまま機能し、コード変更は不要と確認した。COHR再生成
（データコミット`3896f7393`）でshares_diluted/basicを是正: 2009年度
60,164→60,164,000/59,334→59,334,000、2010年度61,504→61,504,000/
60,304→60,304,000、2011年度63,612→63,612,000/62,211→62,211,000。
net_income/eps_diluted/eps_basicは変化なし、NI≈EPS×Sharesの恒等式が
3年度とも1%未満の誤差で成立するようになったことを確認（修正前は約
1000倍の乖離）。105銘柄フローズン入力比較でCOHR(2009-2011)以外は0件
差分、他15年次・四半期データも変化なしを確認。`report_consistency_
check.py` NG=0・WARN 81件（変化なし）、pytest 519 passed/2 known
failed（MSFT/NVDA、既知・本修正と無関係）を確認。TANUKI VALUATIONの
`get_diluted_shares()`は直近1年度のみを参照し2009-2011年度は無関係
（実測値でも変化なしを確認）、STONKS SILOはCOHRがstonks_silo=falseで
追跡対象外のため、いずれも影響なしと確定。
...
#### 調査結果（根本原因調査、チャット記録）
`company_facts.json`のform/accn/end_dateを時系列で突合した結果、決算期
変更は2回のみ（12月31日→4月30日〈2019-2020頃、10-K accn `0001554795-
20-000206`〉・4月30日→12月31日〈2024年、10-KT accn `0001641172-25-
001892`、正式な移行期報告書〉）で確定し、**3段階目の決算期変更は存在
しない**と判明した。

直近10-K（filed 2026-03-19、accn `0001628280-26-019861`）が12月・4月
両クラスタに同時出現していた理由は、決算期変更に伴うSEC開示規則
（Regulation S-X Article 3-06等）により旧4月決算最終フル年度
（2023-05-01〜2024-04-30）の損益計算書・キャッシュフロー計算書を比較列
として含める義務があるためと確認した（実際に94個のXBRLファクトが
start=2023-05-01/end=2024-04-30で当該accn配下に存在することを確認）。
正常な開示要件であり、決算期がさらに変更されている証跡ではない。

`detect_fiscal_end_month()`=12月・`detect_fiscal_anchor_date()`=4月30日
（主anchor）という不一致自体も、`determine_fiscal_year()`が主anchor・
extra_anchorsを区別なく全候補から最小距離を採用する対称探索設計のため、
計算結果に一切影響しないことを確認した（「事故的な正しさ」ではなく、
設計上必然的に無害）。リスクが顕在化しうるのはいずれかのeraの適格年度数
がmin_support=2を下回りextra_anchorsから脱落するケースのみだが、RCATは
両era（4月:5年分・12月:8年分以上）とも十分な件数があり今後も増加のみの
ため、このリスクは実質的にゼロと判定した。

#### 副産物として新規発見・登録した実害
調査中に、当初の懸念とは別種の**現在進行形の実害**を発見した:
RCATの決算期変更移行期スタブ（2024-05-01〜2024-12-31、8ヶ月間、10-KTで
正式報告済み）が、annual_2024.json（旧4月決算12ヶ月分）にもannual_
2025.json（新12月決算12ヶ月分）にも一切含まれず完全に欠落している。
根本原因は`common/sec_data/fetcher.py::_fetch_submissions_for_cik()`の
`relevant_forms`に`10-KT`・`10-QT`が含まれておらずis_own_data判定が
恒常的にFalseになること（ticker非依存の設計欠落）。詳細・対応方針は
[[FETCHER-10KT-10QT-FORM-EXCLUSION-1]]として独立登録した。

#### 対応方針
完了。3段階目の決算期変更懸念は誤りと確認。残る実害
（[[FETCHER-10KT-10QT-FORM-EXCLUSION-1]]）は独立タスクへ切り出し済み。
...
### ✅ [LAYER3-IONQ-REVENUE-2022Q1-ANOMALY-1] IONQのRevenuesタグ2022年Q1データが事業規模に対して不自然に大きい — 2026-08-30、`[[SOFI-TICKER-RESTRICTIONS-NOT-MIGRATED-1]]`（2026-07-24完了）の副作用で解消済みと確認し移設
**状態:** 完了・陳腐化のため移設（2026-08-30、バックログ検証ラウンド3）。
本エントリの着手条件は「[[SOFI-TICKER-RESTRICTIONS-NOT-MIGRATED-1]]の対応と
一体で解消」だったが、同エントリは2026-07-24にコミット46d05a542で完了して
おり、その完了記録自体に「IONQ・SOFI由来のrevenue/gross_profit異常値解消分」
と明記されている（IONQのrevenue_concept固定オーバーライドが実装され、
本エントリと同一原因〈2022年10-KのRevenuesタグがSPAC調達金$1,235Mを誤タグ〉
が解消された）。2026-08-30時点で`common/sec_data/data/IONQ/quarterly_2022Q1.json`
の実データを確認したところ、revenue=1,953,000（旧比較値2,186,000と近い正常な
小規模値）であり、当初報告されていた異常値1,070,233,000（489倍の乖離）は
現存しないことを確認した。着手条件の充足に伴う解消が本エントリ側へ反映
されないまま open のまま残っていた「移設漏れ」と判断し移設する。
**優先度:** 低〜中→クローズ
**分類:** データ品質 / SEC提出データ異常
**登録日:** 2026-07-24
**発見:** フェーズC実装、TTM系列全体での105銘柄回帰

### ✅ [LAYER3-ANCHOR-MISSING-LOOKBACK-WINDOW-1] TTM系列の古いanchor（H1YTD逆算による先行四半期復元）がlayer3_builder.pyで欠落 — 2026-08-30、実装済みと確認し移設
**状態:** 完了・陳腐化のため移設（2026-08-30、バックログ検証ラウンド3）。
本エントリは「`_build_missing_quarter_implied_entries()`相当の機能を
layer3_builder.pyに実装する必要がある」と対応方針に記載していたが、
`common/sec_data/layer3_builder.py`を確認したところ、行728のコメントに
本エントリのID自体（`[[LAYER3-ANCHOR-MISSING-LOOKBACK-WINDOW-1]]対応`）が
明記された上で、行778に`_build_missing_quarter_implied_entries()`関数が
実装済みであることを確認した（normalizer.py版のアルゴリズムを、先頭欠落
パターンのみに限定した派生版として移植。適用対象フィールドも
`MISSING_QUARTER_IMPLIED_FIELDS`として実装済み）。対応方針で要求されていた
実装が既に完了しているにもかかわらず本エントリがopenのまま残っていた
「移設漏れ」と判断し移設する。
**優先度:** 中→クローズ
**分類:** バグ / 要調査
**登録日:** 2026-07-24
...
### ✅ [LAYER3-TTM-TEST-SUITE-SHAPE-MISMATCH-1] tests/test_ttm_calculator.pyの既存テストが新store形状でクラッシュする — 2026-08-30、テスト修正済みと確認し移設
**状態:** 完了・陳腐化のため移設（2026-08-30、バックログ検証ラウンド3）。
本エントリは「既存単体テスト3件が新store形状でAttributeErrorによりクラッシュ
する」と記載していたが、2026-08-30時点で`pytest tests/test_ttm_calculator.py`
を実行したところ7件全て合格（0失敗）することを確認した。テストコードは
既に新store形状（`{"fields": {name: {"entries": [...]}}}`）に対応済みで
あり、記載されていたクラッシュは現存しない。
**優先度:** 中→クローズ
**分類:** テスト / 技術的負債
**登録日:** 2026-07-24
**発見:** フェーズC実装、既存テストスイート実行確認

### ✅ [SPAC-STUB-PERIOD-FIELD-SPLIT-1] BBAI/RDW 2020年度のPredecessor/Successor期間混在、PL/CF系は既にNone化済みで対応不要 — 2026-08-30、本文が自ら「クローズ扱い」と結論済みのため形式的にも移設
**状態:** 完了・記録として移設（2026-08-30、バックログ検証ラウンド3）。
本エントリは2026-08-01の個別調査で「PL/CF系は既存の
[[PERIOD-LENGTH-VALIDATION-GAP-1]]フィルタにより安全側にNone化済み、
追加対応は不要」と結論しており、着手条件欄にも「本エントリはクローズ扱い。
記録として残置」と自ら明記していた（BS系の実害のみ
[[SPAC-SHELL-BS-ENTITY-MIXING-1]]として別途独立登録済み）。2026-08-30時点で
`common/sec_data/data/BBAI/annual_2020.json`・`RDW/annual_2020.json`の
revenue値を確認し、いずれもNoneのままであることを再確認した。内容は
既に完了・クローズと自己結論していたにもかかわらず`### [`（未完了）
ヘッダのまま残っていた「移設漏れ」と判断し、形式を実態に合わせて移設する。
**優先度:** 低（記録のみ・追加対応不要）→クローズ
**分類:** データ品質 / SPAC会計特有の期間混在（確認完了）
**登録日:** 2026-07-31
**訂正日:** 2026-08-01

### ✅ [AVGO-2015-DATA-THIN-1] AVGO(2015)がCIK拡張対象年度範囲外にもかかわらずデータが薄い（bs=2項目・pl=0項目・is_own_data=False） — 2026-08-30、本文が自ら「原因確定によりクローズ」と結論済みのため形式的にも移設
**状態:** 完了・記録として移設（2026-08-30、バックログ検証ラウンド4）。
本エントリは2026-08-05の原因調査で「DELL型の申告ギャップではなく、
cik_history.jsonのAVGO旧CIK登録が無関係な買収先企業を指している疑い」
という、より根本的な問題を発見し、詳細・対応方針の選択肢を
[[AVGO-CIK-HISTORY-WRONG-LEGACY-CIK-1]]として分離登録した上で、本文中に
「状態: 原因確定・[[AVGO-CIK-HISTORY-WRONG-LEGACY-CIK-1]]へ統合」「本
エントリは原因確定によりクローズ」と自ら明記していた。2026-08-30時点で
後続エントリ[[AVGO-CIK-HISTORY-WRONG-LEGACY-CIK-1]]がBACKLOG.mdに正しく
存在し、対応方針（案A/B/C）が引き続き未決定のまま適切に追跡されている
ことを確認した。内容は既に完了・クローズと自己結論していたにもかかわらず
`### [`（未完了）ヘッダのまま残っていた「移設漏れ」と判断し、形式を実態に
合わせて移設する。
**優先度:** 中→クローズ
**分類:** データ品質 / 要調査
**登録日:** 2026-08-05
...
### ✅ [RCAT-FCF-5YR-AVG-ACTUAL-3YR-1] RCATのget_fcf_5yr_avg()が実質2021-2023年の3年平均になっておりDCFのFCFベース値計算に構造的な実害がある — 2026-08-30、後続調査で実際の計算経路はTTM系列でありその根本原因が解消済みと確認し移設
**状態:** 完了・陳腐化のため移設（2026-08-30、バックログ検証ラウンド4）。
本エントリは自らの実装前シミュレーションで「RCATの本番FCF計算は
reader.py::get_fcf_5yr_avg()（年次ファイルベース）を一切使用しておらず、
実際にはTTM系列（common/sec_data/ttm/RCAT_ttm_series.json）を優先採用
している」ことを確認し、着手条件を「単独での緊急対応は不要」とした上で、
TTM系列側の検証を[[RCAT-TTM-SERIES-CONTINUING-DISCONTINUED-UNCHECKED-1]]
へ分離した。同エントリはさらに根本原因（calc_ttm_series()の日付連続性
チェック欠如）を[[TTM-CALC-QUARTER-CONTIGUITY-UNCHECKED-1]]へ切り出し、
同エントリは2026-08-02に実装完了（`_last4_is_contiguous()`新設）済みで、
その完了記録に「RCAT・KULRのIV影響はΔIV=$0と実測確定」と明記されている。
本エントリが依拠する実際の計算経路（TTM系列）の根本原因が既に解消済みと
確認できたため移設する。
**優先度:** 高（登録時）→低（実装前シミュレーションの結果、実害なしと判明）→クローズ
**分類:** バグ / 確定・現在進行形の実害・DCF計算に直結
**登録日:** 2026-08-02
**発見:** [[OPERATING-CASH-FLOW-CONTINUING-DISCONTINUED-GAP-1]]実害確認調査（チャット記録）

### ✅ [RCAT-TTM-SERIES-CONTINUING-DISCONTINUED-UNCHECKED-1] RCATの本番FCF計算が使うTTM系列の継続/非継続事業タグ扱いが未検証 — 2026-08-30、根本対応の完了により解消済みと確認し移設
**状態:** 完了・陳腐化のため移設（2026-08-30、バックログ検証ラウンド4）。
本エントリは自らの根本原因調査で、真因は継続/非継続タグの扱いではなく
calc_ttm_series()の日付連続性チェック欠如という、より一般的な設計欠陥
であると確定し、[[TTM-CALC-QUARTER-CONTIGUITY-UNCHECKED-1]]として切り
出した上で「根本原因の恒久対応は同エントリ側で行う」と明記していた。
同エントリは2026-08-02に実装完了（`_last4_is_contiguous()`新設・
calc_ttm_series()へ挿入、コミット`e2892a91f`/`426b4fa2f`）しており、
完了記録に「RCAT・KULRのIV影響はΔIV=$0と実測確定」「①RCAT型（標準タグの
一時的空白、RCATのみ）: 既報告済み・IV影響はΔIV=$0で確定済み（今回の
スキャンでも再確認）」と明記されている。本エントリが求めていた検証・
対応は完了済みと判断し移設する。
**優先度:** 高（登録時）→中（根本原因確定の結果、現時点のIV実害はゼロと判明）→クローズ
**分類:** 調査要 / 確認済み実害箇所の可能性
**登録日:** 2026-08-02
...
### ✅ [MARKETDATA-AS-IS-AUDIT-PY-OMITTED-1] INPUT_DATA_AS_IS.md 1-B節の「11ファイル」がcommon/sec_data/audit.pyを見落としていた — 2026-08-30、記載通り対応完了済みと確認し移設
**状態:** 完了・移設漏れのため移設（2026-08-30、🟢有効判定項目の診断結論再検証ラウンド16）。
本エントリは登録時点から自ら「優先度: 低（記録のみ、ドキュメント訂正で
対応完了）」と明記しており、実質的にクローズ済みの記録専用エントリ
だった。`INPUT_DATA_AS_IS.md`を実測したところ、1-B節が実際に
「実測12ファイル」へ訂正され、`common/sec_data/audit.py`
（`audit_beta_drift()`）が表に追加され、総数訂正（11→12件）の記述も
本エントリのID（`[[MARKETDATA-AS-IS-AUDIT-PY-OMITTED-1]]`）付きで
明記されていることを確認した。対応内容は本文の「#### 対応」欄の記載
通りに実施済みであり、`### [`（未完了）ヘッダのまま残っていた
「移設漏れ」と判断し、形式を実態に合わせて移設する。
**優先度:** 低（記録のみ）→クローズ
**分類:** ドキュメント正確性 / 調査精度
**登録日:** 2026-08-07
